...
export function actionResponseContract(control, sourceContractHash) {
  ...
  const physicalCommon = [
    ...
    'PRE_REPAIR_GATE obligatorio para toda implementación física: después de materializar todos los cambios deterministas y antes de registrar IMPLEMENTED o iniciar la batería final, ejecuta exactamente una vez npm run quality:repair. Es un corrector local y determinista: normaliza CRLF/BOM en archivos de texto tocados, aplica ESLint --fix solo sobre código fuente tocado no generado y regenera módulos packages/contracts cuando cambió su generador o un artefacto generated. No forma parte de validation_commands porque puede escribir; los validadores finales permanecen read-only/fail-closed. No puede inventar semántica ni ocultar hallazgos no auto-reparables. Solo con PASS y READY_FOR_VALIDATION: SI se permite registrar IMPLEMENTED y continuar sin volver al chat; si devuelve FAIL, ese es un gate de evidencia real.',
  ...
  if (type === 'EJECUTAR_IMPLEMENTACION') {
    ...
    return [
      ...
      `Cuando todos los cambios físicos estén materializados, ejecuta exactamente una vez npm run quality:repair. Solo con PASS y READY_FOR_VALIDATION: SI lleva ${instanceRecordPath} a IMPLEMENTED con evidencia disponible y ejecuta una sola transacción final fail-fast con las validation_commands autorizadas, sin inyectar validadores documentales globales por rutina. Si quality:repair falla, conserva el estado previo y detén la transacción en ese gate. Si toda la evidencia exigida es local, puede continuar directamente a VERIFIED tras PASS. Si alguna validación necesita evidencia remota sobre código publicado, completa primero el tramo local, realiza el commit/push mínimo de materialización mientras ${instanceRecordPath} permanece IMPLEMENTED y ejecuta después el tramo remoto contra ese SHA antes de VERIFIED.`,
