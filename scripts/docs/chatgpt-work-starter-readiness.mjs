...
function queueLines(registry) {
  ...
  return `- ${current.position}/${execution.sequence.length}: ${current.package_id} -> ${current.next_action.type} -> ${current.next_action.target} | CURRENT_EXECUTABLE_WORK=${work.kind}:${work.id}`;
}
