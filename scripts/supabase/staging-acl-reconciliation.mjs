...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const [referencePath, observedPath, outputPrefix] = process.argv.slice(2);
  if (!referencePath || !observedPath || !outputPrefix) throw new Error('Usage: node staging-acl-reconciliation.mjs reference.json observed.json output-prefix');
  const { staging } = readEnvironmentBindings();
  const plan = buildPlan({ reference: JSON.parse(fs.readFileSync(referencePath, 'utf8')),
    observed: JSON.parse(fs.readFileSync(observedPath, 'utf8')), projectRef: staging.project_ref, owner: staging.owner });
  fs.writeFileSync(`${outputPrefix}.json`, `${JSON.stringify(plan, null, 2)}\n`);
  fs.writeFileSync(`${outputPrefix}.sql`, renderRepairSql(plan));
  console.log(JSON.stringify({ revokes: plan.revokes.length, defaults: plan.defaults_to_revoke.length, target: plan.project_ref, mutations_executed: false }));
}
