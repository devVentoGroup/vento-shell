...
4. **`TI-DOM-011`**: la fila que asigna `Implementación Supabase de cualquier mecanismo nuevo` a `NEXO-REMISSIONS-001::CONDITIONAL_IMPLEMENTATION_SCOPE` queda sustituida. `NEXO-REMISSIONS-001` no es autoridad transversal de recuperación tecnológica. Cualquier implementación nueva que implique migraciones o cambios físicos en Supabase queda bajo **BLOQUE R y el paquete de implementación propietario que resulte explícitamente autorizado**, conservando `TI-DOM-011` como contrato documental de recuperación y sin autorizar implementación en esta tarea.
