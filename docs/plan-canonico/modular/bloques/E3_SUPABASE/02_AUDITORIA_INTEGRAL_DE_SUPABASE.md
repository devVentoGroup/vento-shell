...
**Fecha de preparación documental:** 2026-07-29
**Bloque propietario:** BLOQUE E3 — Arquitectura canónica de datos y gobierno integral de Supabase
**Marcador exacto que reemplaza:** `### [ ] SUPA-AUD-023 — Crear mapa proceso → datos → RPC → eventos → aplicaciones`
**Tarea anterior:** `SUPA-AUD-022 — Crear mapa objeto → capacidad empresarial preliminar → propietario actual → consumidores actuales` — APROBADA
**Siguiente tarea:** `SUPA-AUD-024 — Clasificar riesgos críticos, altos, medios y deuda técnica`
**Proyecto observado:** `vento-os-dev` — `clzdpinthhtknkmefsxx`
**Repositorio canónico:** `devVentoGroup/vento-shell` — rama `main`; commit remoto observado `a465917c81c76fc4422be1eef7dfae11fb8d0351`
**Tipo de tarea:** consolidación analítica read-only de procesos, datos, funciones, eventos y aplicaciones; sin DDL, DML, migraciones, cambios en Supabase, código, contratos físicos, RLS, Realtime, Edge Functions, cron ni despliegues

#### 1. Objetivo

Crear una matriz integral que conecte los 69 procesos canónicos con su implementación de datos actualmente observable y con sus contratos funcionales aprobados:
...
APLICACIÓN PROPIETARIA
    ↓
DATOS FÍSICOS ACTUALES O AUSENCIA EXPLÍCITA
    ↓
RPC, FUNCIÓN O AUTOMATISMO ACTUAL
    ↓
EVENTOS EMPRESARIALES DEFINIDOS Y TRANSPORTE REAL
    ↓
APLICACIONES CONSUMIDORAS DIRECTAS Y CONDICIONALES
    ↓
ESTADO DE COBERTURA + SEMILLAS DE RIESGO PARA SUPA-AUD-024
```

El artefacto producido es:

```text
SUPABASE-PROCESS-DATA-RPC-EVENT-APPLICATION-MAP-023@1.0.0
```

Esta tarea describe el estado actual y sus diferencias frente al contrato funcional. No diseña todavía schemas objetivo, RPC definitivas, outbox, topics, migraciones ni severidad de riesgos.

#### 2. Regla de aceleración sin pérdida de calidad

`SUPA-AUD-023` utiliza una sola matriz como fuente de evidencia para dos resultados consecutivos:

1. cerrar el mapa proceso-datos-RPC-eventos-aplicaciones;
2. dejar en cada fila semillas de riesgo estructuradas para que `SUPA-AUD-024` clasifique severidad sin repetir el levantamiento.

La aceleración se obtiene por reutilización de evidencia, no por aprobar dos tareas al mismo tiempo ni por reducir el detalle. `SUPA-AUD-024` permanece reservada hasta la aprobación expresa de esta tarea.

#### 3. Fuentes congeladas y continuidad

| Fuente                                             | Corte o responsabilidad                                                                                                   |
| -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `SUPA-AUD-022_APROBADA_PARA_REEMPLAZAR.md`         | SHA-256 `61cdd8150cb0861524ad1ede15fefdeb4ddcc7498465286072e390aa51cd9c55`; objetos, propietarios y consumidores actuales |
| `04A_REGISTRO_CANONICO_DE_REQUISITOS_DE_PRUEBA.md` | SHA-256 `2959dd3b5220b90258e325d9ee53c1a9b69134a94b7ae0ac304f462a421ea5b5`; 4.749 requisitos hasta `SUPA-AUD-022`         |
| `PROC-CAT-003`                                     | identidades `VPROC-0001` a `VPROC-0069`                                                                                   |
| `PROC-CAT-005`                                     | una propietaria canónica por proceso                                                                                      |
| `PROC-CAT-006`                                     | consumidoras directas y condicionales                                                                                     |
| `PROC-CAT-015`                                     | 69 contratos de entrada; 427 grupos obligatorios y 423 condicionales                                                      |
| `PROC-CAT-016`                                     | 69 contratos de salida; 381 grupos canónicos, 178 proyecciones y 271 grupos de evidencia                                  |
| `PROC-CAT-017`                                     | 395 definiciones de evento canónico, entre cuatro y seis por proceso                                                      |
| Supabase remoto                                    | relaciones y funciones consultadas read-only; nueve schemas gobernados por Vento                                          |
| GitHub remoto                                      | contratos documentales y código indexado; sin escritura                                                                   |

La cabecera remota continúa administrativamente detrás de los artefactos locales aprobados. Se conserva el último artefacto local aprobado como base de continuidad y el remoto como fuente de identidades materializadas.

#### 4. Separaciones obligatorias

```text
CONTRATO DE ENTRADA/SALIDA ≠ TABLA O COLUMNA EXISTENTE
PROCESO ≠ TABLA
PROCESO ≠ RPC
RPC ≠ APLICACIÓN PROPIETARIA
TRIGGER ≠ EVENTO EMPRESARIAL
EVENTO DEFINIDO ≠ EVENTO MATERIALIZADO
REALTIME ≠ OUTBOX TRANSACCIONAL
CRON SUCCEEDED ≠ PROCESO COMPLETADO
CONSUMIDORA CONDICIONAL ≠ CONSUMIDORA SIEMPRE AUTORIZADA
SISTEMA EXTERNO ≠ PROPIETARIA INTERNA
```

Una fila puede citar varios objetos porque un proceso utiliza maestros, transacciones, ledgers, proyecciones y evidencia. Eso no autoriza propiedad múltiple ni escrituras cruzadas.
...
| Campo                       | Regla                                                              |
| --------------------------- | ------------------------------------------------------------------ |
| `process_id`                | `VPROC-*` exacto e inmutable                                       |
| `process_name`              | referencia aprobada, sin sustituir el identificador                |
| `owner_application`         | valor exacto de `PROC-CAT-005`                                     |
| `current_data_objects`      | objetos existentes calificados o ausencia explícita                |
| `current_rpc_or_automation` | funciones observadas o `NONE_LOCATED`                              |
| `canonical_event_namespace` | `VPROC-####.EVT-*` de `PROC-CAT-017`                               |
| `current_event_signal`      | tabla, trigger, webhook o señal actual cuando existe               |
| `canonical_transport_state` | estado de materialización física del evento                        |
| `direct_consumers`          | lista exacta de `PROC-CAT-006`                                     |
| `conditional_consumers`     | lista exacta separada                                              |
| `current_coverage_state`    | estado de cobertura física permitido                               |
| `risk_seeds`                | entrada estructurada para `SUPA-AUD-024`, sin severidad anticipada |

#### 6. Reconciliación global

##### 6.1. Cobertura funcional y técnica

| Dimensión                                                        |                   Resultado |
| ---------------------------------------------------------------- | --------------------------: |
| procesos canónicos                                               |                **69 de 69** |
| identificadores cubiertos                                        | `VPROC-0001` a `VPROC-0069` |
| propietarias canónicas                                           |                      **10** |
| contratos de entrada aprobados                                   |                      **69** |
| contratos de salida aprobados                                    |                      **69** |
| definiciones de evento aprobadas                                 |                     **395** |
| procesos con alguna función o automatismo relacionado localizado |                      **45** |
| procesos sin función/comando canónico localizado                 |                      **24** |
| procesos con transporte canónico `VPROC-*` materializado         |                       **0** |
| relaciones Vento heredadas de `SUPA-AUD-022`                     |                     **379** |
| funciones Vento disponibles para clasificación                   |                     **347** |
| triggers Vento existentes                                        |                     **196** |
| tablas publicadas en Realtime                                    |                       **6** |
| Edge Functions activas                                           |                      **24** |
| cron jobs activos                                                |                       **7** |

##### 6.2. Distribución por propietaria

| Aplicación | Procesos propietarios |
| ---------- | --------------------: |
| `anima`    |                 **1** |
| `aura`     |                 **2** |
| `fogo`     |                 **6** |
...
##### 6.3. Distribución por cobertura actual

| Estado                | Procesos |
| --------------------- | -------: |
| `EXTERNAL_FRAGMENTED` |    **2** |
| `FRAGMENTED_CURRENT`  |   **10** |
| `FUTURE_OR_PARTIAL`   |    **3** |
| `IMPLEMENTED_CURRENT` |    **6** |
| `NO_CANONICAL_SOURCE` |   **20** |
| `PARTIAL_CURRENT`     |   **28** |
| **Total**             |   **69** |

Los estados no representan severidad. Su criticidad, probabilidad, exposición, urgencia y tratamiento se decidirán exclusivamente en `SUPA-AUD-024`.

#### 7. Mapa completo proceso → datos → RPC → eventos → aplicaciones

| Proceso                                                                                                                           | Propietaria | Datos actuales                                                                                                                                                                                                        | RPC, función o automatismo actual                                                                                                                                                                                                                       | Eventos                                                                                                                                                    | Aplicaciones                                                                                                               | Cobertura             | Semillas de riesgo para `SUPA-AUD-024`                                                                                 |
| --------------------------------------------------------------------------------------------------------------------------------- | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | --------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `VPROC-0001` — Gobernar decisiones empresariales con registro, alcance, responsable, compromisos y seguimiento                    | `viso`      | SIN_FUENTE_DEDICADA; referencias dispersas en documentos y soporte administrativo                                                                                                                                     | NONE_LOCATED                                                                                                                                                                                                                                            | `VPROC-0001.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `viso`; directas —; condicionales `anima`, `nexo`, `fogo`, `origo`, `pulso`, `numera`, `aura`, `pass`          | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC`                                                    |
| `VPROC-0002` — Mantener una estructura organizativa y jurídica coherente entre empresas, marcas, establecimientos, sedes y áreas  | `viso`      | public.sites; public.areas; public.area_kinds; public.site_*; public.cost_centers                                                                                                                                     | upsert_operational_checkin_point; upsert_site_operational_role (helpers parciales)                                                                                                                                                                      | `VPROC-0002.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `viso`; directas `shell`, `anima`, `nexo`, `fogo`, `origo`, `pulso`, `numera`, `aura`, `pass`; condicionales — | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE`                                                                         |
| `VPROC-0003` — Gobernar responsabilidades, políticas, delegaciones y límites de decisión mediante versiones vigentes              | `viso`      | public.app_operation_policies; public.app_permissions; public.roles; public.role_permissions; public.role_capabilities; public.operational_*                                                                          | has_effective_permission_v1; permission_scope_matches; upsert_app_screen_registry; promote_app_screen_to_navigation                                                                                                                                     | `VPROC-0003.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `viso`; directas `shell`, `anima`, `nexo`, `fogo`, `origo`, `pulso`, `numera`, `aura`, `pass`; condicionales — | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE`                                                                         |
| `VPROC-0004` — Coordinar compromisos y transferencias de trabajo entre negocios, sedes y áreas                                    | `viso`      | SIN_FUENTE_DEDICADA de compromisos y handoffs interáreas                                                                                                                                                              | NONE_LOCATED                                                                                                                                                                                                                                            | `VPROC-0004.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `viso`; directas —; condicionales `anima`, `nexo`, `fogo`, `origo`, `pulso`, `numera`, `aura`, `pass`          | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC`                                                    |
| `VPROC-0005` — Planear dotación y ejecutar selección sin mezclar necesidad laboral, candidato y trabajador activo                 | `viso`      | talento.vacancies; talento.candidates; talento.candidate_profiles; talento.applications; talento.application_*; talento.interviews; talento.offers; talento.medical_evaluations                                       | talento.bootstrap_my_candidate; talento.submit_application; talento.advance_application_stage; talento.schedule_interview; talento.confirm_interview; talento.respond_to_offer; talento.handoff_to_anima; talento.transfer_candidate_to_employee        | `VPROC-0005.EVT-*` en PROC-CAT-017; señal actual: talento.application_events y application_stage_history; transporte canónico: `NOT_MATERIALIZED`          | propietaria `viso`; directas —; condicionales `anima`, `numera`                                                            | `FUTURE_OR_PARTIAL`   | `NO_CANONICAL_EVENT_TRANSPORT, DEFERRED_OR_FUTURE, PARTIAL_SOURCE, OWNER_IMPLEMENTATION_GAP`                           |
| `VPROC-0006` — Orquestar vinculación, expediente, incorporación, preparación y habilitación inicial de la persona                 | `viso`      | public.staff_invitations; public.employees; public.employee_sites; public.employee_areas; public.documents; talento.candidate_employee_links; talento.preboarding_tasks                                               | reconcile_staff_invitations; handle_new_user; upsert_employee_site_operational_profile; talento.handoff_to_anima                                                                                                                                        | `VPROC-0006.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `viso`; directas `anima`, `shell`; condicionales `nexo`, `fogo`, `origo`, `pulso`, `numera`, `aura`            | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE`                                                                         |
| `VPROC-0007` — Administrar asignaciones laborales y programación publicada con historial y revisión controlada                    | `viso`      | public.employee_shifts; public.employee_sites; public.employee_areas; public.site_operational_roles; viso.site_*; viso.shift_generation_*                                                                             | apply_operational_profile_to_shift; notify_shift_published; run_shift_runtime_processor; scheduled_auto_close_shift                                                                                                                                     | `VPROC-0007.EVT-*` en PROC-CAT-017; señal actual: public.shift_runtime_events y notificación de turno; transporte canónico: `NOT_MATERIALIZED`             | propietaria `viso`; directas `anima`, `shell`; condicionales `nexo`, `fogo`, `origo`, `pulso`, `numera`                    | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE`                                                                         |
| `VPROC-0008` — Capturar asistencia como hechos inmutables y corregirla mediante decisiones auditables                             | `anima`     | public.attendance_logs; public.attendance_breaks; public.attendance_shift_events; public.attendance_sync_conflicts; public.asistencia_logs                                                                            | start_attendance_break; end_attendance_break; sync_attendance_events; close_open_attendance_day_end; register_shift_departure_event; resolve_attendance_shift_id                                                                                        | `VPROC-0008.EVT-*` en PROC-CAT-017; señal actual: public.attendance_shift_events; transporte canónico: `NOT_MATERIALIZED`                                  | propietaria `anima`; directas `viso`, `numera`, `shell`; condicionales `nexo`, `fogo`, `origo`, `pulso`                    | `IMPLEMENTED_CURRENT` | `NO_CANONICAL_EVENT_TRANSPORT`                                                                                         |
| `VPROC-0009` — Gestionar novedades, ausencias, permisos y reemplazos como casos laborales completos                               | `viso`      | public.employee_shifts; public.staff_manual_calendar_events; public.attendance_shift_events; SIN_FUENTE_DEDICADA de caso laboral                                                                                      | apply_operational_profile_to_shift; register_shift_departure_event_autoclose (sin RPC de caso)                                                                                                                                                          | `VPROC-0009.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `viso`; directas `anima`, `shell`; condicionales `nexo`, `fogo`, `origo`, `pulso`, `numera`                    | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC`                                               |
| `VPROC-0010` — Preparar y reconciliar el paquete autorizado para pagos y beneficios laborales                                     | `numera`    | SIN_FUENTE_DEDICADA de nómina/beneficios; consume empleados, asistencia y novedades                                                                                                                                   | numera_current_period_summary (consulta, no nómina)                                                                                                                                                                                                     | `VPROC-0010.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `numera`; directas `viso`, `anima`; condicionales —                                                            | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE`                                                                              |
| `VPROC-0011` — Orquestar retiro laboral, devolución, revocación de accesos y cierre documental                                    | `viso`      | public.employees; public.employee_devices; public.employee_permissions; public.staff_invitations; public.shared_operational_devices; SIN_ORQUESTADOR_DE_OFFBOARDING                                                   | reconcile_staff_invitations; anonymize_user_personal_data (sin orquestador integral)                                                                                                                                                                    | `VPROC-0011.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `viso`; directas `shell`, `anima`, `nexo`, `fogo`, `origo`, `pulso`, `numera`, `aura`; condicionales —         | `FRAGMENTED_CURRENT`  | `NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE`                                                                      |
| `VPROC-0012` — Gestionar riesgos, inspecciones, controles preventivos y acciones correctivas                                      | `viso`      | SIN_FUENTE_RELACIONAL_DIRECTA; documentos, activos y soporte son auxiliares                                                                                                                                           | NONE_LOCATED                                                                                                                                                                                                                                            | `VPROC-0012.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `viso`; directas `anima`, `nexo`, `fogo`, `origo`, `pulso`; condicionales `numera`                             | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC`                                                    |
| `VPROC-0013` — Gestionar incidentes, accidentes y emergencias con respuesta inmediata y expediente posterior                      | `viso`      | SIN_FUENTE_RELACIONAL_DIRECTA de expediente SST; soporte y documentos son auxiliares                                                                                                                                  | NONE_LOCATED                                                                                                                                                                                                                                            | `VPROC-0013.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `viso`; directas `anima`, `nexo`, `fogo`, `origo`, `pulso`; condicionales `numera`                             | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC`                                                    |
| `VPROC-0014` — Ejecutar controles de higiene, inocuidad y cumplimiento mediante procedimientos versionados                        | `viso`      | SIN_FUENTE_RELACIONAL_DIRECTA de controles de higiene/inocuidad                                                                                                                                                       | NONE_LOCATED                                                                                                                                                                                                                                            | `VPROC-0014.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `viso`; directas `nexo`, `fogo`, `origo`, `pulso`, `anima`; condicionales —                                    | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC`                                                    |
| `VPROC-0015` — Gobernar el ciclo de vida de productos, presentaciones, unidades y equivalencias                                   | `nexo`      | public.products; public.product_categories; public.product_uom_profiles; public.product_inventory_profiles; public.product_images; public.product_site_settings; public.product_sku_*                                 | apply_master_product_identity_batch; apply_master_presentation_version_batch; apply_master_product_site_batch; generate_product_sku; generate_inventory_sku; set_product_sku                                                                            | `VPROC-0015.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `nexo`; directas `fogo`, `origo`, `pulso`, `numera`, `pass`, `aura`, `viso`; condicionales —                   | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, CROSS_DOMAIN_BOUNDARY`                                                  |
| `VPROC-0016` — Gestionar desarrollo, prueba, aprobación, publicación y versión de recetas                                         | `fogo`      | public.recipes; public.recipe_cards; public.recipe_steps; public.recipe_outputs; public.recipe_site_uses; public.product_site_production_routes                                                                       | fogo_recipe_area_options; can_access_recipe_scope; validate_recipe_outputs; validate_recipe_site_use                                                                                                                                                    | `VPROC-0016.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `fogo`; directas `nexo`, `pulso`, `numera`; condicionales `origo`                                              | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE`                                                                         |
| `VPROC-0017` — Publicar oferta y disponibilidad desde una definición gobernada hacia todos los canales                            | `pulso`     | pass.catalog_*; pass.commercial_*; pass.sell_products_by_site; public.sell_products_by_site; public.product_site_settings                                                                                             | pass.seed_catalog_items_from_sell_products; pass.sync_main_collection_categories; pass.reconcile_catalog_items_after_collection_status_change; get_site_order_status                                                                                    | `VPROC-0017.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `pulso`; directas `pass`, `aura`, `nexo`, `fogo`, `numera`; condicionales —                                    | `FRAGMENTED_CURRENT`  | `NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, CROSS_DOMAIN_BOUNDARY`                                               |
| `VPROC-0018` — Mantener especificaciones, alérgenos, restricciones y criterios de calidad del producto                            | `nexo`      | public.products; public.product_inventory_profiles; public.product_site_settings; SIN_FUENTE_EXPLÍCITA de alérgenos                                                                                                   | apply_master_inventory_profile_batch; validate_product_site_production_route; validate_product_site_production_location                                                                                                                                 | `VPROC-0018.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `nexo`; directas `fogo`, `origo`, `pulso`, `pass`, `aura`, `viso`; condicionales —                             | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE`                                                                         |
| `VPROC-0019` — Capturar y priorizar necesidades de compra mediante una entrada única y trazable                                   | `origo`     | public.product_request_policies; public.restock_requests; public.purchase_orders; SIN_ENTRADA_ÚNICA de necesidad de compra                                                                                            | apply_master_request_policy_batch; apply_master_request_policy_rules_batch; apply_master_request_policy_units_batch                                                                                                                                     | `VPROC-0019.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `origo`; directas `numera`; condicionales `nexo`, `fogo`, `pulso`                                              | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE`                                                                         |
| `VPROC-0020` — Comparar proveedores y condiciones con evidencia suficiente para decidir                                           | `origo`     | public.suppliers; public.product_suppliers; public.procurement_agreed_prices; public.procurement_supplier_product_costs; public.v_procurement_price_book                                                              | apply_master_supplier_purchase_batch; rebuild_procurement_supplier_product_costs; normalize_procurement_receipt_costs                                                                                                                                   | `VPROC-0020.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `origo`; directas `numera`, `viso`; condicionales `nexo`, `fogo`                                               | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE`                                                                         |
| `VPROC-0021` — Aprobar y emitir compras separando flujo ordinario, urgencia y excepción                                           | `origo`     | public.purchase_orders; public.purchase_order_items                                                                                                                                                                   | NONE_LOCATED para aprobar/emitir compra                                                                                                                                                                                                                 | `VPROC-0021.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `origo`; directas `nexo`, `numera`; condicionales `fogo`                                                       | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC`                                               |
| `VPROC-0022` — Recibir compras, verificar conformidad y resolver diferencias sin separar recepción física, documental y económica | `origo`     | public.procurement_receptions; public.procurement_reception_items; public.inventory_entries; public.inventory_entry_items; public.purchase_orders                                                                     | receive_purchase_order; origo_mark_inventory_entry_corrected; origo_reverse_inventory_entry; rebuild_procurement_supplier_product_costs                                                                                                                 | `VPROC-0022.EVT-*` en PROC-CAT-017; señal actual: triggers de recepción/costo; sin evento VPROC; transporte canónico: `NOT_MATERIALIZED`                   | propietaria `origo`; directas `nexo`, `numera`; condicionales `fogo`, `pulso`                                              | `FRAGMENTED_CURRENT`  | `NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, CROSS_DOMAIN_BOUNDARY`                                               |
| `VPROC-0023` — Gobernar sedes, LOC, zonas, posiciones y condiciones de almacenamiento                                             | `nexo`      | public.inventory_locations; public.inventory_location_positions; public.inventory_location_product_catalog                                                                                                            | generate_location_code; assign_inventory_stock_to_location; assign_inventory_stock_to_position                                                                                                                                                          | `VPROC-0023.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `nexo`; directas `fogo`, `origo`, `pulso`, `viso`; condicionales —                                             | `IMPLEMENTED_CURRENT` | `NO_CANONICAL_EVENT_TRANSPORT`                                                                                         |
| `VPROC-0024` — Registrar ingreso, ubicación y reubicación mediante movimientos correlacionados                                    | `nexo`      | public.inventory_entries; public.inventory_entry_items; public.inventory_movements; public.inventory_transfers; public.inventory_stock_by_*                                                                           | assign_inventory_stock_to_location; assign_inventory_stock_to_position; upsert_inventory_stock_by_location; upsert_inventory_stock_by_uom_profile                                                                                                       | `VPROC-0024.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `nexo`; directas `origo`, `fogo`, `pulso`, `numera`; condicionales —                                           | `IMPLEMENTED_CURRENT` | `NO_CANONICAL_EVENT_TRANSPORT`                                                                                         |
| `VPROC-0025` — Retirar, consumir o trasladar existencias conservando unidad, conversión, origen y destino                         | `nexo`      | public.inventory_movements; public.inventory_transfers; public.inventory_transfer_items; public.inventory_stock_by_*; public.inventory_lpns; public.inventory_lpn_items                                               | consume_inventory_stock_by_uom_profile; consume_inventory_stock_from_positions; assign_inventory_stock_to_position                                                                                                                                      | `VPROC-0025.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `nexo`; directas `fogo`, `pulso`, `origo`, `numera`; condicionales —                                           | `IMPLEMENTED_CURRENT` | `NO_CANONICAL_EVENT_TRANSPORT`                                                                                         |
| `VPROC-0026` — Contar como observación, investigar diferencias y ajustar mediante decisión separada                               | `nexo`      | public.inventory_count_sessions; public.inventory_count_lines; public.inventory_count_line_entries; public.inventory_stock_by_*                                                                                       | create_inventory_count_session_with_lines; close_inventory_count_session; apply_inventory_count_adjustments; reconcile_inventory_stock_positions_for_count                                                                                              | `VPROC-0026.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `nexo`; directas `numera`, `viso`; condicionales `fogo`, `pulso`, `origo`                                      | `IMPLEMENTED_CURRENT` | `NO_CANONICAL_EVENT_TRANSPORT`                                                                                         |
| `VPROC-0027` — Gestionar condición, vencimiento, cuarentena, merma, pérdida, frío y disposición                                   | `nexo`      | public.inventory_movements; public.inventory_stock_by_*; SIN_FUENTE_DEDICADA de vencimiento/cuarentena/cadena de frío                                                                                                 | NONE_LOCATED para condición/cuarentena/disposición                                                                                                                                                                                                      | `VPROC-0027.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `nexo`; directas `fogo`, `pulso`, `origo`, `viso`, `numera`; condicionales —                                   | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC`                                               |
| `VPROC-0028` — Ejecutar abastecimiento interno de solicitud a recepción con cantidades conciliables por etapa                     | `nexo`      | public.restock_requests; public.restock_request_items; public.restock_request_item_picks; public.restock_item_fulfillments; public.remission_*                                                                        | apply_restock_receipt; apply_restock_shipment; apply_restock_shipment_from_picks; create_remission_shipment_from_fulfillments; confirm_remission_shipment_receipt; mark_restock_request_in_transit; reverse_restock_request; split_restock_request_item | `VPROC-0028.EVT-*` en PROC-CAT-017; señal actual: triggers y estados de restock/remission; sin evento VPROC; transporte canónico: `NOT_MATERIALIZED`       | propietaria `nexo`; directas `fogo`, `origo`, `pulso`, `numera`; condicionales —                                           | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, CROSS_DOMAIN_BOUNDARY`                                                  |
| `VPROC-0029` — Gestionar identidad, ubicación, custodia, préstamo y transferencia de activos                                      | `nexo`      | public.asset_items; public.asset_groups; public.asset_movements; public.asset_documents; public.product_asset_profiles; public.product_asset_transfer_events                                                          | NONE_LOCATED                                                                                                                                                                                                                                            | `VPROC-0029.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `nexo`; directas `viso`, `numera`, `anima`, `origo`; condicionales —                                           | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC`                                               |
| `VPROC-0030` — Gestionar mantenimiento, reparación, garantía, repuesto y disposición de activos                                   | `nexo`      | public.asset_maintenance_records; public.product_asset_maintenance_events; public.asset_items                                                                                                                         | NONE_LOCATED                                                                                                                                                                                                                                            | `VPROC-0030.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `nexo`; directas `origo`, `numera`, `viso`; condicionales `anima`                                              | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC`                                               |
| `VPROC-0031` — Gestionar disponibilidad de vehículos, combustible, kilometraje e incidencias                                      | `nexo`      | SIN_FUENTE_DEDICADA de vehículos, combustible y kilometraje                                                                                                                                                           | NONE_LOCATED                                                                                                                                                                                                                                            | `VPROC-0031.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `nexo`; directas `viso`, `numera`, `anima`, `origo`; condicionales —                                           | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC`                                                    |
| `VPROC-0032` — Controlar entrega, tenencia, retorno, pérdida y completitud de reutilizables y contenedores                        | `nexo`      | public.asset_groups; public.asset_items; public.inventory_lpns; public.inventory_lpn_items; SIN_CICLO_DE_RETORNO_DEDICADO                                                                                             | NONE_LOCATED                                                                                                                                                                                                                                            | `VPROC-0032.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `nexo`; directas `fogo`, `pulso`, `numera`; condicionales —                                                    | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC`                                               |
| `VPROC-0033` — Planear producción desde demanda, inventario, capacidad, prioridad y fecha requerida                               | `fogo`      | public.production_requests; public.production_request_items; viso.demand_forecasts; viso.demand_history_hourly; public.product_site_production_routes                                                                 | NONE_LOCATED para planificación aprobada                                                                                                                                                                                                                | `VPROC-0033.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `fogo`; directas `nexo`, `origo`, `pulso`, `numera`, `viso`; condicionales —                                   | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC`                                               |
| `VPROC-0034` — Preparar materiales y ejecutar producción contra una versión aprobada                                              | `fogo`      | public.production_batches; public.production_batch_consumptions; public.production_batch_outputs; public.production_batch_packages; public.recipes                                                                    | fogo_create_production_batch_from_recipe; fogo_create_real_production_batch; set_production_batch_code                                                                                                                                                  | `VPROC-0034.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `fogo`; directas `nexo`, `numera`, `pulso`; condicionales —                                                    | `IMPLEMENTED_CURRENT` | `NO_CANONICAL_EVENT_TRANSPORT`                                                                                         |
| `VPROC-0035` — Inspeccionar y decidir liberación, retención, rechazo o corrección de producto                                     | `fogo`      | public.production_batches; public.production_batch_outputs; SIN_FUENTE_DEDICADA de decisión de liberación/calidad                                                                                                     | NONE_LOCATED para liberación/retención/rechazo                                                                                                                                                                                                          | `VPROC-0035.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `fogo`; directas `nexo`, `pulso`, `viso`; condicionales `numera`                                               | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC`                                                    |
| `VPROC-0036` — Empacar, etiquetar y almacenar producto terminado con trazabilidad preservada                                      | `fogo`      | public.production_batch_packages; public.printing_label_templates; public.inventory_lpns                                                                                                                              | generate_lpn_code; set_production_batch_code                                                                                                                                                                                                            | `VPROC-0036.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `fogo`; directas `nexo`, `pulso`, `numera`; condicionales —                                                    | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE`                                                                         |
| `VPROC-0037` — Gestionar reproceso, aprovechamiento, rendimiento, merma y cierre productivo                                       | `fogo`      | public.production_batches; public.production_batch_consumptions; public.production_batch_outputs; public.inventory_movements; SIN_LEDGER_DE_REPROCESO                                                                 | NONE_LOCATED para reproceso/cierre                                                                                                                                                                                                                      | `VPROC-0037.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `fogo`; directas `nexo`, `numera`, `viso`; condicionales —                                                     | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC`                                               |
| `VPROC-0038` — Gestionar servicio en mesa de apertura a cierre con pedido, preparación, entrega, pago y conciliación              | `pulso`     | pos.pos_sessions; pos.pos_session_orders; pos.pos_tables; pos.pos_zones; pos.pos_*modifiers; pos.pos_payments; public.orders; public.order_items                                                                      | create_order_checkout_draft; create_scheduled_order_checkout_draft; ensure_order_conversation; sync_order_fulfillment_state                                                                                                                             | `VPROC-0038.EVT-*` en PROC-CAT-017; señal actual: public.order_status_events; triggers de fulfillment; transporte canónico: `NOT_MATERIALIZED`             | propietaria `pulso`; directas `fogo`, `nexo`, `numera`, `pass`; condicionales —                                            | `FRAGMENTED_CURRENT`  | `NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, CROSS_DOMAIN_BOUNDARY`                                               |
| `VPROC-0039` — Gestionar venta de mostrador o para llevar con entrega y cobro correlacionados                                     | `pulso`     | public.orders; public.order_items; public.order_item_options; pos.pos_sessions; pos.pos_session_orders; pos.pos_payments                                                                                              | create_order_checkout_draft; create_scheduled_order_checkout_draft; process_order_payment; sync_order_fulfillment_state                                                                                                                                 | `VPROC-0039.EVT-*` en PROC-CAT-017; señal actual: public.order_status_events; triggers de fulfillment; transporte canónico: `NOT_MATERIALIZED`             | propietaria `pulso`; directas `fogo`, `nexo`, `numera`, `pass`; condicionales —                                            | `FRAGMENTED_CURRENT`  | `NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, CROSS_DOMAIN_BOUNDARY`                                               |
| `VPROC-0040` — Normalizar pedidos de canales externos y transferirlos al proceso interno con reconciliación                       | `pulso`     | public.orders; public.order_status_events; public.order_conversations; public.order_messages; public.pulso_daily_sales_import_*; public.pulso_external_sales_item_mappings                                            | create_order_checkout_draft; pulso_post_daily_sales_import; pulso_sync_external_sales_item_mapping_product_id; ensure_order_conversation                                                                                                                | `VPROC-0040.EVT-*` en PROC-CAT-017; señal actual: public.order_status_events; import batches/rows; transporte canónico: `NOT_MATERIALIZED`                 | propietaria `pulso`; directas `fogo`, `nexo`, `numera`; condicionales `pass`, `aura`                                       | `EXTERNAL_FRAGMENTED` | `NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, EXTERNAL_RECONCILIATION, CROSS_DOMAIN_BOUNDARY`                      |
| `VPROC-0041` — Gestionar cotización, aprobación, capacidad, producción, facturación y entrega de catering o venta B2B             | `pulso`     | public.orders; public.order_billing_requests; SIN_FUENTE_DEDICADA de cotización/contrato B2B o catering                                                                                                               | create_order_checkout_draft; sync_order_billing_request_status (sin RPC B2B integral)                                                                                                                                                                   | `VPROC-0041.EVT-*` en PROC-CAT-017; señal actual: public.order_status_events y billing requests; transporte canónico: `NOT_MATERIALIZED`                   | propietaria `pulso`; directas `aura`, `fogo`, `nexo`, `origo`, `numera`; condicionales —                                   | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC`                                                    |
| `VPROC-0042` — Gestionar modificación, sustitución, cancelación, anulación y devolución sin confundir sus efectos                 | `pulso`     | public.orders; public.order_status_events; payments.transactions; pos.pos_payments; public.inventory_movements; public.loyalty_*                                                                                      | update_order_operational_state; update_order_gift_operational_state; mark_payment_transaction_status; sync_order_fulfillment_state                                                                                                                      | `VPROC-0042.EVT-*` en PROC-CAT-017; señal actual: public.order_status_events; efectos distribuidos; transporte canónico: `NOT_MATERIALIZED`                | propietaria `pulso`; directas `fogo`, `nexo`, `numera`, `pass`; condicionales —                                            | `FRAGMENTED_CURRENT`  | `NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, CROSS_DOMAIN_BOUNDARY`                                               |
| `VPROC-0043` — Cobrar, confirmar pago y emitir soporte fiscal mediante contrato conciliable                                       | `pulso`     | payments.transactions; payments.webhook_events; pos.pos_payments; public.order_billing_requests; public.internal_pos_documents                                                                                        | process_order_payment; mark_payment_transaction_status; checkout_get_payment_transaction; checkout_record_payment_webhook_event; reconcile_expired_payment_checkouts                                                                                    | `VPROC-0043.EVT-*` en PROC-CAT-017; señal actual: payments.webhook_events y estados de transacción; transporte canónico: `NOT_MATERIALIZED`                | propietaria `pulso`; directas `numera`, `pass`; condicionales —                                                            | `FRAGMENTED_CURRENT`  | `NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, CROSS_DOMAIN_BOUNDARY`                                               |
| `VPROC-0044` — Cerrar caja y conciliar ventas, pagos, efectivo, diferencias y responsables                                        | `pulso`     | pos.pos_cash_shifts; pos.pos_cash_movements; pos.pos_payments; public.pulso_daily_sales_import_*; public.internal_pos_documents                                                                                       | generate_daily_internal_pos_documents; generate_manual_daily_internal_pos_documents; preview_daily_internal_pos_documents; pulso_post_daily_sales_import                                                                                                | `VPROC-0044.EVT-*` en PROC-CAT-017; señal actual: cash shifts/movements e import batches; transporte canónico: `NOT_MATERIALIZED`                          | propietaria `pulso`; directas `numera`, `viso`; condicionales —                                                            | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE`                                                                         |
| `VPROC-0045` — Identificar cliente y administrar fidelización mediante ledgers y consentimientos separados                        | `pass`      | public.users; pass.loyalty_transactions; pass.loyalty_rewards; pass.loyalty_redemptions; club.wallet_*; club.subscriptions; club.entitlements; club.earn_events; public.wallet_*                                      | grant_loyalty_points; process_loyalty_earning; update_loyalty_balance; award_loyalty_points_external; club.apply_redeem_debit; club.reverse_redeem_debit; club.get_my_wallet                                                                            | `VPROC-0045.EVT-*` en PROC-CAT-017; señal actual: pass.loyalty_transactions; club.earn_events; club.wallet_ledger; transporte canónico: `NOT_MATERIALIZED` | propietaria `pass`; directas `pulso`, `aura`, `viso`, `numera`; condicionales —                                            | `FRAGMENTED_CURRENT`  | `NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, CROSS_DOMAIN_BOUNDARY`                                               |
| `VPROC-0046` — Gestionar reclamo, devolución, compensación y aprendizaje de causa                                                 | `pulso`     | public.support_tickets; public.support_messages; public.order_conversations; public.order_messages; public.user_feedback; FUENTE_FRAGMENTADA                                                                          | support_ticket_is_visible_to_current_employee; ensure_order_conversation; mark_order_conversation_read (sin RPC de resolución)                                                                                                                          | `VPROC-0046.EVT-*` en PROC-CAT-017; señal actual: support_messages y order_messages; transporte canónico: `NOT_MATERIALIZED`                               | propietaria `pulso`; directas `pass`, `numera`, `viso`, `aura`; condicionales —                                            | `FRAGMENTED_CURRENT`  | `NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, NO_CANONICAL_COMMAND_RPC, CROSS_DOMAIN_BOUNDARY`                     |
| `VPROC-0047` — Gestionar reservas, eventos y comunicaciones al cliente con capacidad y consentimiento                             | `pulso`     | pass.site_business_hours; pass.site_delivery_slots; pass.site_schedule_exceptions; public.orders; SIN_FUENTE_DEDICADA de reserva/evento                                                                               | get_order_delivery_slots; pass.get_order_reschedule_slots_admin; pass.reschedule_scheduled_order_admin; pass.resolve_scheduled_order_contact_admin                                                                                                      | `VPROC-0047.EVT-*` en PROC-CAT-017; señal actual: site schedule exceptions y order status; transporte canónico: `NOT_MATERIALIZED`                         | propietaria `pulso`; directas `pass`, `aura`; condicionales `fogo`, `nexo`                                                 | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE`                                                                              |
| `VPROC-0048` — Planear ruta, vehículo, carga, secuencia y restricciones antes del despacho                                        | `nexo`      | public.remission_dispatch_runs; public.site_supply_routes; SIN_FUENTE_DEDICADA de vehículo/asignación de ruta                                                                                                         | NONE_LOCATED para plan de ruta                                                                                                                                                                                                                          | `VPROC-0048.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `nexo`; directas `pulso`, `fogo`, `origo`, `viso`, `numera`; condicionales —                                   | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC`                                               |
| `VPROC-0049` — Ejecutar ruta y confirmar entrega, rechazo, novedad o retorno con prueba suficiente                                | `nexo`      | public.remission_shipments; public.remission_receipts; public.remission_exceptions; public.inventory_transfers; public.order_delivery_sessions                                                                        | mark_restock_request_in_transit; confirm_remission_shipment_receipt; update_delivery_portal_state                                                                                                                                                       | `VPROC-0049.EVT-*` en PROC-CAT-017; señal actual: remission shipment/receipt state; transporte canónico: `NOT_MATERIALIZED`                                | propietaria `nexo`; directas `pulso`, `fogo`, `origo`, `numera`, `viso`; condicionales —                                   | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE`                                                                         |
| `VPROC-0050` — Integrar entrega de tercero con seguimiento, prueba y conciliación interna                                         | `pulso`     | public.order_delivery_sessions; pass.delivery_quotes; pass.delivery_addresses; pass.delivery_distance_rates; app_private.delivery_pin_secrets                                                                         | create_order_delivery_courier_link; get_delivery_portal_data_by_token; get_order_delivery_pin; update_delivery_portal_state; override_order_delivery_confirmation                                                                                       | `VPROC-0050.EVT-*` en PROC-CAT-017; señal actual: order_delivery_sessions y delivery portal state; transporte canónico: `NOT_MATERIALIZED`                 | propietaria `pulso`; directas `pass`, `numera`, `nexo`; condicionales `aura`                                               | `EXTERNAL_FRAGMENTED` | `NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, EXTERNAL_RECONCILIATION, CROSS_DOMAIN_BOUNDARY`                      |
| `VPROC-0051` — Registrar hechos económicos desde eventos operativos y soportes correlacionados                                    | `numera`    | public.numera_expenses; public.numera_periods; public.cost_centers; public.product_cost_events; public.internal_pos_documents; public.internal_transfer_variances; hechos operativos referenciados                    | numera_current_period_summary; viso_accounting_dashboard; estimate_internal_price_unit; resolve_internal_transfer_price                                                                                                                                 | `VPROC-0051.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `numera`; directas `viso`, `nexo`, `fogo`, `origo`, `pulso`; condicionales `anima`, `aura`, `pass`             | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, CROSS_DOMAIN_BOUNDARY`                                                  |
| `VPROC-0052` — Gestionar obligación, aprobación y pago a proveedor con conciliación bancaria                                      | `numera`    | public.purchase_orders; public.procurement_receptions; public.numera_expenses; SIN_LEDGER_DE_CUENTAS_POR_PAGAR                                                                                                        | NONE_LOCATED para obligación/pago a proveedor                                                                                                                                                                                                           | `VPROC-0052.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `numera`; directas `origo`; condicionales `viso`                                                               | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC`                                                    |
| `VPROC-0053` — Gestionar cartera, cobro, recaudo, aplicación y diferencia                                                         | `numera`    | public.orders; payments.transactions; public.order_billing_requests; SIN_LEDGER_DE_CUENTAS_POR_COBRAR                                                                                                                 | checkout_find_payment_transaction_by_reference; mark_payment_transaction_status (sin RPC de cartera)                                                                                                                                                    | `VPROC-0053.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `numera`; directas `pulso`; condicionales `viso`, `aura`                                                       | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC`                                                    |
| `VPROC-0054` — Gestionar costos, distribución, presupuesto, cierre y rentabilidad con reglas versionadas                          | `numera`    | public.numera_periods; public.numera_expenses; public.numera_cost_center_budgets; public.cost_centers; public.numera_cost_center_monthly_summary; public.product_cost_events                                          | numera_current_period_summary; viso_accounting_dashboard; resolve_internal_transfer_price; price_restock_request_internal_transfer                                                                                                                      | `VPROC-0054.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `numera`; directas `viso`, `nexo`, `fogo`, `origo`, `pulso`, `aura`; condicionales —                           | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE`                                                                         |
| `VPROC-0055` — Gestionar limpieza, inspección, mantenimiento, plagas, servicios y cierre de novedades de instalaciones            | `nexo`      | public.asset_maintenance_records; public.support_tickets; SIN_ORDEN_DE_TRABAJO_DE_INSTALACIÓN                                                                                                                         | support_ticket_is_visible_to_current_employee (sin RPC de instalación)                                                                                                                                                                                  | `VPROC-0055.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `nexo`; directas `viso`, `anima`, `fogo`, `origo`, `pulso`, `numera`; condicionales —                          | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC`                                                    |
| `VPROC-0056` — Gestionar contenido y promociones desde solicitud y aprobación hasta publicación y retiro                          | `aura`      | public.announcements; public.app_content_blocks; public.website_blocks; public.website_items; pass.commercial_collections; club.cashback_rules; AURA_DIFERIDA                                                         | NONE_LOCATED; solo triggers de actualización/sincronización                                                                                                                                                                                             | `VPROC-0056.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `aura`; directas `pulso`, `pass`, `viso`, `numera`; condicionales —                                            | `FUTURE_OR_PARTIAL`   | `NO_CANONICAL_EVENT_TRANSPORT, DEFERRED_OR_FUTURE, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC, OWNER_IMPLEMENTATION_GAP` |
| `VPROC-0057` — Convertir consultas y oportunidades de canales digitales en casos comerciales trazables                            | `aura`      | public.order_conversations; public.support_tickets; public.user_feedback; SIN_CRM_DE_OPORTUNIDADES; AURA_DIFERIDA                                                                                                     | ensure_order_conversation; mark_order_conversation_read (sin RPC CRM)                                                                                                                                                                                   | `VPROC-0057.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `aura`; directas `pulso`, `pass`, `viso`, `numera`; condicionales —                                            | `FUTURE_OR_PARTIAL`   | `NO_CANONICAL_EVENT_TRANSPORT, DEFERRED_OR_FUTURE, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC, OWNER_IMPLEMENTATION_GAP` |
| `VPROC-0058` — Gestionar solicitudes e incidentes tecnológicos con diagnóstico, prioridad, resolución y conocimiento              | `viso`      | public.support_tickets; public.support_messages; public.support_ticket_reads; public.employee_devices; public.shared_operational_devices                                                                              | support_ticket_is_visible_to_current_employee; notify_support_message_inserted                                                                                                                                                                          | `VPROC-0058.EVT-*` en PROC-CAT-017; señal actual: support_messages trigger de notificación; transporte canónico: `NOT_MATERIALIZED`                        | propietaria `viso`; directas `anima`, `shell`, `nexo`, `fogo`, `origo`, `pulso`, `numera`, `aura`, `pass`; condicionales — | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE`                                                                         |
| `VPROC-0059` — Gestionar el ciclo de acceso tecnológico desde solicitud hasta revocación y verificación                           | `viso`      | public.users; public.employees; public.roles; public.role_permissions; public.employee_permissions; public.app_permissions; public.shared_operational_*; public.staff_invitations; public.context_simulation_sessions | get_effective_context_v1; has_effective_permission_v1; start_context_simulation_v1; stop_context_simulation_v1; current_shared_operational_device_v1; sign_shared_device_action; verify_shared_device_action_signature                                  | `VPROC-0059.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `viso`; directas `shell`, `anima`, `nexo`, `fogo`, `origo`, `pulso`, `numera`, `aura`, `pass`; condicionales — | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, CROSS_DOMAIN_BOUNDARY`                                                  |
| `VPROC-0060` — Gestionar documentos y evidencia desde creación hasta disposición con metadatos y custodia                         | `viso`      | public.documents; public.document_types; public.required_document_rules; public.asset_documents; Storage buckets; FUENTES_DISTRIBUIDAS                                                                                | NONE_LOCATED para ciclo documental integral                                                                                                                                                                                                             | `VPROC-0060.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `viso`; directas `anima`, `nexo`, `fogo`, `origo`, `pulso`, `numera`, `aura`, `pass`; condicionales —          | `FRAGMENTED_CURRENT`  | `NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, NO_CANONICAL_COMMAND_RPC, CROSS_DOMAIN_BOUNDARY`                     |
| `VPROC-0061` — Gestionar medición, análisis, decisión de mejora y verificación de resultado                                       | `numera`    | viso.demand_forecasts; viso.demand_history_hourly; public.numera_cost_center_monthly_summary; vistas/reportes; SIN_CASO_DE_MEJORA                                                                                     | numera_current_period_summary; viso_accounting_dashboard (sin RPC de mejora)                                                                                                                                                                            | `VPROC-0061.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `numera`; directas `viso`, `nexo`, `fogo`, `origo`, `pulso`, `aura`, `pass`, `anima`; condicionales —          | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC`                                               |
| `VPROC-0062` — Gestionar continuidad desde detección hasta operación mínima, recuperación, reconciliación y aprendizaje           | `viso`      | public.support_tickets; public.app_runtime_settings; public.app_update_policies; public.account_deletion_requests; SIN_REGISTRO_INTEGRAL_DE_CONTINUIDAD                                                               | archive_finished_order_conversations; reconcile_expired_payment_checkouts; close_stale_open_attendance_shifts (automatismos parciales)                                                                                                                  | `VPROC-0062.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `viso`; directas `shell`, `anima`, `nexo`, `fogo`, `origo`, `pulso`, `numera`, `aura`, `pass`; condicionales — | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE`                                                                              |
| `VPROC-0063` — Gestionar riesgos empresariales como registro versionado de riesgo, tratamiento y seguimiento                      | `viso`      | SIN_REGISTRO_EMPRESARIAL_DE_RIESGOS                                                                                                                                                                                   | NONE_LOCATED                                                                                                                                                                                                                                            | `VPROC-0063.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `viso`; directas `numera`, `nexo`, `fogo`, `origo`, `pulso`, `aura`, `pass`, `anima`; condicionales —          | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC`                                                    |
| `VPROC-0064` — Gestionar relaciones con asesores y autoridades conservando obligaciones, vencimientos y evidencia interna         | `viso`      | public.documents; public.required_document_rules; SIN_CASO_DE_ASESOR_AUTORIDAD                                                                                                                                        | NONE_LOCATED                                                                                                                                                                                                                                            | `VPROC-0064.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `viso`; directas —; condicionales `numera`, `nexo`, `fogo`, `origo`, `pulso`, `aura`, `pass`, `anima`          | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC`                                                    |
| `VPROC-0065` — Acompañar desempeño y desarrollo con objetivos, retroalimentación, privacidad y decisiones separadas               | `viso`      | SIN_FUENTE_DEDICADA de desempeño y desarrollo                                                                                                                                                                         | NONE_LOCATED                                                                                                                                                                                                                                            | `VPROC-0065.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `viso`; directas `anima`, `numera`; condicionales —                                                            | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC`                                                    |
| `VPROC-0066` — Entregar y controlar elementos de protección desde requisito hasta devolución y evidencia                          | `viso`      | public.asset_items; public.inventory_movements; public.employee_inventory_location_assignments; SIN_CICLO_EPP                                                                                                         | NONE_LOCATED                                                                                                                                                                                                                                            | `VPROC-0066.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `viso`; directas `anima`, `nexo`; condicionales `fogo`, `origo`, `pulso`, `shell`                              | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC`                                                    |
| `VPROC-0067` — Gestionar kits y conjuntos sin confundir kit, activo, LPN o contenedor                                             | `nexo`      | public.asset_groups; public.asset_items; public.inventory_lpns; SIN_ENTIDAD_KIT                                                                                                                                       | NONE_LOCATED                                                                                                                                                                                                                                            | `VPROC-0067.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `nexo`; directas `fogo`, `pulso`, `origo`, `numera`, `viso`; condicionales —                                   | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC`                                                    |
| `VPROC-0068` — Medir satisfacción separando medición, incentivo, reclamo y compensación                                           | `pulso`     | public.user_feedback; SIN_INSTRUMENTO_MUESTRA_SESGO_DEDICADO                                                                                                                                                          | NONE_LOCATED                                                                                                                                                                                                                                            | `VPROC-0068.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `pulso`; directas `pass`, `aura`, `viso`, `numera`; condicionales —                                            | `NO_CANONICAL_SOURCE` | `NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC`                                                    |
| `VPROC-0069` — Gestionar presupuestos con versión, supuestos, aprobación, vigencia, consumo, proyección y desviación              | `numera`    | public.numera_cost_center_budgets; public.numera_periods; public.cost_centers; public.numera_cost_center_monthly_summary                                                                                              | numera_current_period_summary (consulta, sin comando presupuestal integral)                                                                                                                                                                             | `VPROC-0069.EVT-*` en PROC-CAT-017; señal actual: sin outbox/event stream VPROC materializado; transporte canónico: `NOT_MATERIALIZED`                     | propietaria `numera`; directas `viso`, `nexo`, `fogo`, `origo`, `pulso`, `aura`; condicionales —                           | `PARTIAL_CURRENT`     | `NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE`                                                                         |

#### 8. Reconciliación de datos

##### 8.1. Procesos sin fuente canónica

Un proceso conserva `NO_CANONICAL_SOURCE` cuando no existe una entidad que pueda representar su instancia, estado, corrección y cierre completos. Objetos auxiliares no se elevan a fuente por inferencia.

Los casos se transfieren a `SUPA-AUD-024`, `SUPA-ARC-002` y `SUPA-ARC-003` con su fila completa.

##### 8.2. Procesos parciales

`PARTIAL_CURRENT` significa que existen maestros, transacciones o cálculos útiles, pero falta al menos una de estas dimensiones:

- instancia integral del proceso;
- estado y transición gobernados;
- comando autorizado;
- evidencia de cierre;
- conciliación interaplicación;
- fuente única del resultado.

##### 8.3. Procesos fragmentados

`FRAGMENTED_CURRENT` y `EXTERNAL_FRAGMENTED` conservan todos los objetos actuales y no eligen una fuente falsa. La resolución corresponde a `SUPA-ARC-016`, `SUPA-ARC-017` y la transición posterior.

##### 8.4. Estructuras futuras

Los objetos `talento.*` y la cobertura prevista de AURA se registran como evidencia técnica o funcional parcial. No convierten automáticamente a TALENTO en aplicación propietaria ni a AURA en aplicación operativa.

#### 9. Reconciliación de RPC y automatismos

La base contiene **347 funciones Vento**. Esta tarea no afirma que cada una sea una RPC canónica de proceso.

Clasificaciones obligatorias:

| Clase                   | Significado                                             |
| ----------------------- | ------------------------------------------------------- |
| `PROCESS_COMMAND`       | solicita una transición o efecto empresarial autorizado |
| `PROCESS_QUERY`         | devuelve una proyección o estado sin mutar la fuente    |
| `DOMAIN_HELPER`         | cálculo o resolución interna de un dominio              |
| `VALIDATION_FUNCTION`   | protege integridad o precondición                       |
| `TRIGGER_FUNCTION`      | se ejecuta por trigger; no es API cliente por sí sola   |
| `AUTOMATION_ENTRYPOINT` | ejecutada por Edge, cron, webhook o job                 |
| `NONE_LOCATED`          | no se localizó comando o consulta canónica suficiente   |

Reglas:

1. el registro ejecutable futuro utilizará la firma exacta `<schema>.<nombre>(identity_arguments)`;
2. los nombres de la matriz son referencias legibles y deben resolverse contra `SUPA-AUD-006` antes de generar contratos;
3. una función en `public` no pertenece a PULSO, NEXO o VISO por ubicación;
4. `SECURITY DEFINER`, grants y RLS no sustituyen autorización empresarial;
5. una escritura directa a estado no se acepta como comando canónico;
6. los overloads permanecen firmas distintas;
7. una Edge Function no adquiere propiedad del proceso por usar `service_role`.

#### 10. Reconciliación de eventos

`PROC-CAT-017` define **395 eventos empresariales** asociados a los 69 procesos. En el corte actual:

```text
EVENTOS CANÓNICOS DEFINIDOS                 395
PROCESOS CON NAMESPACE VPROC-####.EVT-*     69
OUTBOX VPROC MATERIALIZADO                   0
TRANSPORTE CANÓNICO VPROC VERIFICADO         0
```

Existen señales técnicas y tablas de historia, entre ellas:

- `talento.application_events` y `talento.application_stage_history`;
- `public.attendance_shift_events`;
- `public.shift_runtime_events`;
- `public.order_status_events`;
- `payments.webhook_events`;
- `pass.loyalty_transactions`;
- `club.earn_events` y `club.wallet_ledger`;
- triggers sobre pedidos, remisiones, asistencia y soporte;
- seis tablas publicadas en Realtime;
- 24 Edge Functions y siete cron jobs.

Estas superficies son evidencia actual, pero no constituyen por sí solas el contrato `VPROC-*` porque no existe todavía una capa física única que garantice:

- identidad y versión de evento;
- productor propietario;
- audiencia aprobada;
- atomicidad con el cambio empresarial;
- deduplicación e idempotencia;
- orden por agregado;
- replay, dead-letter y reconciliación;
- sensibilidad y retención;
- correlación con comando, evidencia y auditoría.

La materialización corresponde a `SUPA-ARC-007`, `SUPA-ARC-019`, `SUPA-ARC-020` y sus transiciones posteriores.

#### 11. Matriz de aplicaciones

| Aplicación | Procesos como propietaria | Procesos consumidos directos | Procesos consumidos condicionales | Total consumido aprobado |
| ---------- | ------------------------: | ---------------------------: | --------------------------------: | -----------------------: |
| `shell`    |                         0 |                           10 |                                 1 |                       11 |
| `anima`    |                         1 |                           21 |                                 6 |                       27 |
| `viso`     |                        20 |                           28 |                                 2 |                       30 |
...
Las cifras expresan vínculos funcionales aprobados, no acceso técnico actual, volumen de datos, número de pantallas ni disponibilidad operativa.

#### 12. Fronteras críticas preservadas

| Frontera                | Regla preservada                                                                       | Destino exacto                                 |
| ----------------------- | -------------------------------------------------------------------------------------- | ---------------------------------------------- |
| VISO ↔ ANIMA            | VISO gobierna vínculo y programación; ANIMA gobierna asistencia y experiencia personal | `SUPA-ARC-016`; `SUPA-ARC-017`                 |
| VISO ↔ TALENTO          | TALENTO es estructura/canal futuro; VISO conserva propiedad de selección y vinculación | `SUPA-AUD-024`; `SUPA-ARC-003`                 |
| NEXO ↔ FOGO             | NEXO gobierna existencias; FOGO receta, lote y ejecución productiva                    | `SUPA-ARC-016`; `SUPA-ARC-017`                 |
| ORIGO ↔ NEXO            | ORIGO acepta comercialmente la recepción; NEXO registra el efecto físico               | `SUPA-ARC-016`; `SUPA-ARC-017`                 |
| PULSO ↔ PASS            | PULSO gobierna pedido/venta; PASS identidad, fidelización y autoservicio               | `SUPA-ARC-016`; `SUPA-ARC-017`                 |
| PULSO ↔ NUMERA          | PULSO gobierna venta/pago/caja; NUMERA efecto económico y conciliación                 | `SUPA-ARC-016`; `SUPA-ARC-017`                 |
| NEXO ↔ PULSO            | NEXO gobierna traslado interno; PULSO entrega comercial al cliente                     | `SUPA-ARC-016`; `SUPA-ARC-017`                 |
| AURA ↔ PULSO/PASS       | AURA gobierna mercadeo diferido; PULSO aplica oferta y PASS presenta beneficio         | `SUPA-AUD-024`; `SUPA-ARC-003`; `SUPA-ARC-016` |
| aplicaciones ↔ terceros | tercero autentica y ejecuta por adaptador; la propietaria Vento conserva el hecho      | `SUPA-ARC-020`; `INT-EXT-001` a `INT-EXT-019`  |

#### 13. Registro estructurado de entrada para `SUPA-AUD-024`

La siguiente tabla no asigna severidad. Conserva conjuntos exhaustivos para que la siguiente tarea clasifique riesgo sin repetir la auditoría.

| ID de conjunto       | Condición                                           | Procesos afectados | Destino documental                             |
| -------------------- | --------------------------------------------------- | -----------------: | ---------------------------------------------- |
| `R-SUPA-AUD-023-001` | Procesos sin fuente canónica actual                 |             **20** | `SUPA-AUD-024`, `SUPA-ARC-002`, `SUPA-ARC-003` |
| `R-SUPA-AUD-023-002` | Procesos con fuente fragmentada o externa           |             **12** | `SUPA-AUD-024`, `SUPA-ARC-016`, `SUPA-ARC-017` |
| `R-SUPA-AUD-023-003` | Procesos futuros o parciales en aplicación diferida |              **3** | `SUPA-AUD-024`, `SUPA-ARC-003`                 |
| `R-SUPA-AUD-023-004` | Procesos sin RPC/comando canónico localizado        |             **24** | `SUPA-AUD-024`, `SUPA-ARC-013`, `SUPA-ARC-016` |
| `R-SUPA-AUD-023-005` | Procesos sin transporte canónico de eventos VPROC   |             **69** | `SUPA-AUD-024`, `SUPA-ARC-007`, `SUPA-ARC-019` |
| `R-SUPA-AUD-023-006` | Fronteras interdominio señaladas                    |             **15** | `SUPA-AUD-024`, `SUPA-ARC-016`, `SUPA-ARC-017` |
| `R-SUPA-AUD-023-007` | Integraciones externas que exigen conciliación      |              **2** | `SUPA-AUD-024`, `SUPA-ARC-020`                 |

Cada conjunto se deriva de las filas individuales. `SUPA-AUD-024` deberá calcular severidad por impacto, probabilidad, exposición, detectabilidad, reversibilidad, alcance de datos y efecto operativo; no solo por el nombre de la condición.

#### 14. Hallazgos y rutas de resolución

| ID                   | Hallazgo                                                                                                 | Resolución exacta                              |
| -------------------- | -------------------------------------------------------------------------------------------------------- | ---------------------------------------------- |
| `B-SUPA-AUD-023-001` | no existe fuente canónica para 20 procesos                                                               | `SUPA-AUD-024`; `SUPA-ARC-002`; `SUPA-ARC-003` |
| `B-SUPA-AUD-023-002` | 24 procesos no tienen comando o RPC canónica localizada                                                  | `SUPA-AUD-024`; `SUPA-ARC-013`; `SUPA-ARC-016` |
| `B-SUPA-AUD-023-003` | los 395 eventos están definidos pero no existe transporte canónico VPROC materializado                   | `SUPA-AUD-024`; `SUPA-ARC-007`; `SUPA-ARC-019` |
| `B-SUPA-AUD-023-004` | triggers, Realtime, webhooks, Edge y cron son superficies heterogéneas sin registro único proceso-evento | `SUPA-AUD-024`; `SUPA-ARC-019`; `SUPA-ARC-020` |
| `B-SUPA-AUD-023-005` | varias fuentes actuales cruzan propietarias y consumidoras sin frontera técnica única                    | `SUPA-AUD-024`; `SUPA-ARC-016`; `SUPA-ARC-017` |
| `B-SUPA-AUD-023-006` | TALENTO y AURA poseen estructuras o propiedad objetivo sin cobertura operativa completa                  | `SUPA-AUD-024`; `SUPA-ARC-003`                 |
| `B-SUPA-AUD-023-007` | no existe artefacto ejecutable único que valide las 69 filas y sus 395 eventos                           | `SHELL-CI-017`; `SUPA-AUD-024`                 |

No se crean tareas nuevas. Todos los hallazgos tienen destino existente y exacto.
...
| ID                  | Regla protegida                                                                                                                                                              | Tipo                                                   | Prioridad | Momento de implementación                        | Destino                                  |
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ | --------- | ------------------------------------------------ | ---------------------------------------- |
| `TREQ-SUPABASE-455` | El mapa proceso-datos-RPC-eventos-aplicaciones deberá cubrir exactamente VPROC-0001 a VPROC-0069 sin omisiones ni duplicados.                                                | contractual + integración + regresión                  | crítica   | implementación de `SUPA-AUD-024` y validación CI | SUPA-AUD-024; SHELL-CI-017               |
| `TREQ-SUPABASE-456` | Cada proceso deberá conservar exactamente la aplicación propietaria aprobada en PROC-CAT-005; schema, tabla, RPC o repositorio no podrán sustituirla.                        | contractual + arquitectura + regresión                 | crítica   | implementación de `SUPA-ARC-003` y validación CI | SUPA-ARC-003; SUPA-ARC-016               |
| `TREQ-SUPABASE-457` | Las consumidoras directas y condicionales de cada proceso deberán coincidir con PROC-CAT-006 y permanecer en listas separadas.                                               | contractual + integración + seguridad + regresión      | crítica   | implementación de `SUPA-ARC-016` y validación CI | SUPA-ARC-016; SHELL-CI-017               |
| `TREQ-SUPABASE-458` | Toda relación de datos actual citada por un proceso deberá resolver a una identidad de objeto de SUPA-AUD-022 o a una ausencia explícita.                                    | contractual + base de datos + regresión                | crítica   | implementación de `SUPA-AUD-024` y validación CI | SUPA-AUD-024; SHELL-CI-017               |
| `TREQ-SUPABASE-459` | Los conceptos funcionales de entrada y salida definidos por PROC-CAT-015 y PROC-CAT-016 no se presentarán como columnas o tablas existentes sin evidencia física.            | contractual + arquitectura + regresión negativa        | crítica   | implementación de `SUPA-ARC-003` y validación CI | SUPA-ARC-003; SUPA-ARC-016               |
| `TREQ-SUPABASE-460` | Todo proceso sin fuente canónica actual deberá declarar NO_CANONICAL_SOURCE y conservar su destino en SUPA-AUD-024 y arquitectura.                                           | contractual + regresión negativa                       | crítica   | implementación de `SUPA-AUD-024` y validación CI | SUPA-AUD-024; SUPA-ARC-002; SUPA-ARC-003 |
| `TREQ-SUPABASE-461` | Una estructura futura, diferida o parcial no podrá certificarse como proceso actual por existir una tabla, schema o aplicación candidata.                                    | contractual + interfaz + regresión negativa            | alta      | implementación de `SUPA-AUD-024` y validación CI | SUPA-AUD-024; SUPA-ARC-003               |
| `TREQ-SUPABASE-462` | Un mismo objeto podrá soportar varios procesos únicamente con un rol explícito por proceso y una sola fuente propietaria.                                                    | contractual + integración + regresión                  | crítica   | implementación de `SUPA-ARC-016` y validación CI | SUPA-ARC-016; SUPA-ARC-017               |
| `TREQ-SUPABASE-463` | Solo la aplicación propietaria podrá mantener el registro autoritativo del proceso; las demás usarán referencias, proyecciones, comandos o eventos.                          | contractual + integración + seguridad + regresión      | crítica   | implementación de `SUPA-ARC-016` y validación CI | SUPA-ARC-016; SUPA-ARC-017               |
| `TREQ-SUPABASE-464` | Una vista, alias, tabla de compatibilidad o proyección no podrá registrarse como fuente independiente del proceso sin una decisión empresarial distinta.                     | contractual + integración + regresión                  | crítica   | implementación de `SUPA-ARC-005` y validación CI | SUPA-ARC-005; SUPA-ARC-016               |
| `TREQ-SUPABASE-465` | Toda RPC o función actual vinculada a un proceso deberá conservar schema, nombre y firma PostgreSQL exacta; el nombre abreviado no bastará para contratos ejecutables.       | RPC + contractual + regresión                          | crítica   | implementación de `SUPA-ARC-013` y validación CI | SUPA-ARC-013; SHELL-CI-017               |
| `TREQ-SUPABASE-466` | Cada función deberá clasificarse como comando, consulta, helper, trigger, validación o automatismo antes de presentarla como RPC de proceso.                                 | RPC + contractual + regresión                          | alta      | implementación de `SUPA-ARC-013` y validación CI | SUPA-ARC-013; SUPA-ARC-016               |
| `TREQ-SUPABASE-467` | Una escritura directa de cliente a tablas de estado no podrá considerarse comando canónico del proceso ni eludir autorización, transición e idempotencia.                    | seguridad + RPC + RLS + regresión                      | crítica   | implementación de `SUPA-ARC-015` y validación CI | SUPA-ARC-015; SUPA-ARC-016; SUPA-ARC-017 |
| `TREQ-SUPABASE-468` | SECURITY DEFINER, grants y RLS deberán evaluarse como controles técnicos separados del propietario y del contrato funcional de la RPC.                                       | seguridad + RPC + RLS + regresión                      | crítica   | implementación de `SUPA-ARC-014` y validación CI | SUPA-ARC-014; SUPA-ARC-015               |
| `TREQ-SUPABASE-469` | Los overloads de una función deberán registrarse y probarse como firmas distintas; agregar o retirar una firma no podrá ocultarse bajo el mismo nombre.                      | RPC + contractual + regresión                          | crítica   | implementación de `SUPA-ARC-013` y validación CI | SUPA-ARC-013; SHELL-CI-017               |
| `TREQ-SUPABASE-470` | Toda Edge Function vinculada a un proceso deberá declarar proceso, propietaria, comando o evento, autenticación, idempotencia, efecto y consumidor.                          | integración + seguridad + idempotencia + regresión     | crítica   | implementación de `SUPA-ARC-020` y validación CI | SUPA-ARC-020; SUPA-AUD-024               |
| `TREQ-SUPABASE-471` | Las 395 definiciones de eventos de PROC-CAT-017 deberán resolver exactamente a uno de los 69 procesos, su propietaria y consumidoras aprobadas.                              | contractual + integración + regresión                  | crítica   | implementación de `SUPA-ARC-007` y validación CI | SUPA-ARC-007; SUPA-ARC-019; SHELL-CI-017 |
| `TREQ-SUPABASE-472` | Ningún evento VPROC se considerará materializado sin un contrato físico versionado, persistencia recuperable y transporte verificable.                                       | contractual + integración + regresión negativa         | crítica   | implementación de `SUPA-ARC-007` y validación CI | SUPA-ARC-007; SUPA-ARC-019               |
| `TREQ-SUPABASE-473` | El cambio empresarial y el registro publicable del evento deberán ser atómicos mediante outbox transaccional o mecanismo equivalente aprobado.                               | base de datos + integración + idempotencia + regresión | crítica   | implementación de `SUPA-ARC-007` y validación CI | SUPA-ARC-007; SUPA-ARC-019               |
| `TREQ-SUPABASE-474` | Un trigger, log, notificación, cambio de updated_at o fila técnica no podrá presentarse como evento empresarial canónico por sí solo.                                        | contractual + integración + regresión negativa         | alta      | implementación de `SUPA-ARC-007` y validación CI | SUPA-ARC-007; SUPA-ARC-013; SUPA-ARC-019 |
| `TREQ-SUPABASE-475` | Toda publicación y suscripción Realtime deberá vincular un evento o proyección permitida, productor, consumidor, filtro, RLS y recuperación.                                 | integración + seguridad + regresión                    | crítica   | implementación de `SUPA-ARC-019` y validación CI | SUPA-ARC-019; SUPA-AUD-024               |
| `TREQ-SUPABASE-476` | Todo webhook o evento externo deberá conservar payload original, firma, identificador externo, correlación, deduplicación y conciliación antes de producir un hecho interno. | integración + seguridad + idempotencia + regresión     | crítica   | implementación de `SUPA-ARC-020` y validación CI | SUPA-ARC-020; INT-EXT-001                |
| `TREQ-SUPABASE-477` | La ejecución exitosa de cron, pg_net o una Edge Function no podrá cerrar un proceso sin comprobar el efecto empresarial propietario.                                         | integración + regresión + E2E                          | crítica   | implementación de `SUPA-ARC-020` y validación CI | SUPA-ARC-020; SUPA-AUD-024               |
| `TREQ-SUPABASE-478` | Productores y audiencias de datos, RPC y eventos deberán permanecer compatibles con la propietaria y consumidoras aprobadas del proceso.                                     | contractual + integración + regresión                  | crítica   | implementación de `SUPA-ARC-016` y validación CI | SUPA-ARC-016; SUPA-ARC-019; SHELL-CI-017 |
| `TREQ-SUPABASE-479` | Una aplicación no declarada como propietaria o consumidora deberá fallar de forma cerrada al consultar, comandar o suscribirse al proceso.                                   | seguridad + integración + regresión                    | crítica   | implementación de `SUPA-ARC-015` y validación CI | SUPA-ARC-015; SUPA-ARC-016; SHELL-CI-017 |
| `TREQ-SUPABASE-480` | Una consumidora condicional solo podrá activarse cuando la condición empresarial aprobada sea demostrable; el silencio no equivale a consumo autorizado.                     | contractual + autorización + integración + regresión   | crítica   | implementación de `SUPA-ARC-016` y validación CI | SUPA-ARC-016; SHELL-CI-017               |
| `TREQ-SUPABASE-481` | Rappi, Shopify, bancos, proveedores, autoridades y otros terceros permanecerán como fuentes o consumidores externos y nunca como propietarias internas del proceso.          | contractual + integración + regresión negativa         | crítica   | implementación de `SUPA-ARC-016` y validación CI | SUPA-ARC-016; SUPA-ARC-020               |
| `TREQ-SUPABASE-482` | Toda escritura cruzada entre aplicaciones o dominios deberá registrarse como contrato o riesgo; una referencia de código no bastará para autorizarla.                        | integración + seguridad + regresión                    | crítica   | implementación de `SUPA-ARC-016` y validación CI | SUPA-ARC-016; SUPA-ARC-017; SUPA-AUD-024 |
| `TREQ-SUPABASE-483` | Cada proceso deberá declarar un estado de cobertura física actual permitido y este no podrá derivarse solo de que exista alguna tabla o función relacionada.                 | contractual + regresión                                | crítica   | implementación de `SUPA-AUD-024` y validación CI | SUPA-AUD-024; SHELL-CI-017               |
| `TREQ-SUPABASE-484` | Los procesos con NO_CANONICAL_SOURCE deberán transferirse íntegramente a SUPA-AUD-024 sin reducción de alcance ni sustitución por objetos auxiliares.                        | contractual + regresión                                | crítica   | implementación de `SUPA-AUD-024` y validación CI | SUPA-AUD-024; SUPA-ARC-002; SUPA-ARC-003 |
| `TREQ-SUPABASE-485` | Los procesos fragmentados, externos o con propiedad técnica no alineada deberán transferirse a SUPA-AUD-024 con sus objetos, fronteras y consumidoras.                       | contractual + arquitectura + regresión                 | crítica   | implementación de `SUPA-AUD-024` y validación CI | SUPA-AUD-024; SUPA-ARC-016; SUPA-ARC-017 |
| `TREQ-SUPABASE-486` | Todo hallazgo del mapa deberá indicar una tarea exacta de resolución y ninguna ausencia podrá quedar como pendiente narrativo.                                               | contractual + regresión                                | crítica   | implementación de `SUPA-AUD-024` y validación CI | SUPA-AUD-024; SHELL-CI-017               |
| `TREQ-SUPABASE-487` | El artefacto deberá conservar orden determinista, hashes de fuentes, conteos y referencias resolubles para detectar drift.                                                   | contractual + estática + regresión                     | crítica   | implementación de `SHELL-CI-017` y validación CI | SHELL-CI-017; SUPA-AUD-024               |
| `TREQ-SUPABASE-488` | Un validador deberá comprobar una fila por VPROC-0001 a VPROC-0069, propietaria válida, consumidoras válidas, cobertura, datos, RPC, eventos y destino de riesgo.            | contractual + estática + integración + regresión       | crítica   | implementación de `SHELL-CI-017` y validación CI | SHELL-CI-017; SUPA-AUD-024               |
| `TREQ-SUPABASE-489` | El registro 04A deberá preservar todas las filas históricas y añadir TREQ-SUPABASE-455 a 490 con catorce columnas, secuencia y relaciones resolubles.                        | contractual + regresión                                | crítica   | implementación de `SHELL-CI-017` y validación CI | SHELL-CI-017; SUPA-AUD-024               |
| `TREQ-SUPABASE-490` | SUPA-AUD-024 deberá consumir las semillas de riesgo de las 69 filas, clasificarlas sin reauditar identidades y conservar todo destino documental.                            | contractual + arquitectura + regresión                 | crítica   | implementación de `SUPA-AUD-024` y validación CI | SUPA-AUD-024                             |

Se incorporan `TREQ-SUPABASE-455` a `TREQ-SUPABASE-490` en el registro `04A` completo.

#### 16. Validaciones del artefacto

El artefacto deberá satisfacer:

- exactamente 69 filas principales;
- rango consecutivo `VPROC-0001` a `VPROC-0069`;
- una propietaria permitida por proceso;
- listas directas y condicionales separadas;
- dato físico calificado o ausencia explícita;
- RPC/automatismo o `NONE_LOCATED`;
- namespace de evento por proceso;
- estado de transporte canónico;
- cobertura física permitida;
- al menos una semilla de riesgo y destino en `SUPA-AUD-024`;
- 36 requisitos nuevos consecutivos;
- preservación íntegra de las 4.749 filas históricas del `04A`.

#### 17. Manifiestos de integridad

| Manifiesto                 | Valor                                                                      |
| -------------------------- | -------------------------------------------------------------------------- |
| base `SUPA-AUD-022`        | SHA-256 `61cdd8150cb0861524ad1ede15fefdeb4ddcc7498465286072e390aa51cd9c55` |
| base `04A`                 | SHA-256 `2959dd3b5220b90258e325d9ee53c1a9b69134a94b7ae0ac304f462a421ea5b5` |
| procesos                   | 69                                                                         |
| propietarias               | 10                                                                         |
| contratos de entrada       | 69                                                                         |
| contratos de salida        | 69                                                                         |
| eventos definidos          | 395                                                                        |
| relaciones Vento heredadas | 379                                                                        |
| funciones Vento heredadas  | 347                                                                        |
| nuevos requisitos          | `TREQ-SUPABASE-455` a `TREQ-SUPABASE-490`                                  |

#### 18. Criterios de aceptación

- [ ] `SUPA-AUD-022` figura aprobada.
- [ ] Existe una fila por cada `VPROC-0001` a `VPROC-0069`.
- [ ] Las 69 propietarias coinciden con `PROC-CAT-005`.
- [ ] Las consumidoras directas y condicionales coinciden con `PROC-CAT-006`.
- [ ] Los datos actuales se resuelven contra `SUPA-AUD-022` o declaran ausencia.
- [ ] Las funciones se presentan como superficie actual, no como contrato objetivo automático.
- [ ] Los 395 eventos de `PROC-CAT-017` quedan vinculados a procesos y aplicaciones.
- [ ] Ningún trigger, webhook, Realtime, Edge o cron se presenta como evento canónico sin contrato.
- [ ] Las coberturas físicas no se confunden con definición funcional.
- [ ] Las semillas de riesgo cubren todas las filas y permanecen sin severidad.
- [ ] Todo hallazgo tiene tarea de resolución exacta.
- [ ] `04A` conserva filas históricas y añade 36 requisitos consecutivos.
- [ ] No se ejecutó DDL, DML, migración, cambio de código ni escritura remota.
- [ ] `SUPA-AUD-024` permanece reservada.

#### 19. Continuidad inmediata
...
→ propietario actual → consumidores actuales
        ↓
TAREA ACTUAL PREPARADA PARA REEMPLAZO
SUPA-AUD-023 — Crear mapa proceso → datos → RPC → eventos → aplicaciones
        ↓
SIGUIENTE TAREA RESERVADA
...
No se inicia `SUPA-AUD-024` hasta la aprobación expresa de `SUPA-AUD-023` y una nueva solicitud de continuidad.
...
**Fecha de preparación documental:** 2026-07-29
**Bloque propietario:** BLOQUE E3 — Arquitectura canónica de datos y gobierno integral de Supabase
**Marcador exacto que reemplaza:** `### [ ] SUPA-AUD-024 — Clasificar riesgos críticos, altos, medios y deuda técnica`
**Tarea anterior:** `SUPA-AUD-023 — Crear mapa proceso → datos → RPC → eventos → aplicaciones` — APROBADA
**Siguiente tarea:** `SUPA-ARC-001 — Definir principios de separación entre esquemas administrados y empresariales`
**Proyecto observado:** `vento-os-dev` — `clzdpinthhtknkmefsxx`
**Repositorio canónico:** `devVentoGroup/vento-shell` — rama `main`; commit observado `939071fc7d4c173e01f15cf10312af0740afe447`
**Tipo de tarea:** consolidación analítica y clasificación read-only de riesgos; sin DDL, DML, migraciones, cambios de RLS, grants, Auth, Storage, Realtime, Edge Functions, cron, secretos, código, despliegues, backfills, retiros ni aceptación de riesgo

#### 1. Objetivo

Cerrar la fase de auditoría integral de Supabase mediante un registro único que consolide los hallazgos de `SUPA-AUD-001` a `SUPA-AUD-023`, asigne una severidad primaria reproducible, preserve la evidencia y determine la puerta documental exacta que deberá superar cada riesgo antes de arquitectura, transición o implementación.

```text
INVENTARIOS Y HALLAZGOS SUPA-AUD-001 A SUPA-AUD-023
        +
MAPA DE 379 OBJETOS Y 69 PROCESOS
        ↓
NORMALIZACIÓN DE RIESGOS SIN DUPLICAR HALLAZGOS
        ↓
16 CRÍTICOS + 20 ALTOS + 10 MEDIOS + 10 DEUDA TÉCNICA
        ↓
GATES Y TAREAS EXACTAS PARA SUPA-ARC Y SUPA-TRANS
```

La clasificación no corrige, acepta ni cierra riesgos. Tampoco convierte una observación en vulnerabilidad explotada cuando la evidencia solo demuestra exposición, inconsistencia o ausencia de control.

#### 2. Artefacto producido

```text
SUPABASE-CONSOLIDATED-RISK-REGISTER-001@1.0.0
```
...
| `processes_covered`             | **69** — `VPROC-0001` a `VPROC-0069`     |
| `source_tasks`                  | **23** — `SUPA-AUD-001` a `SUPA-AUD-023` |
| `unclassified_findings`         | **0**                                    |
| `risks_without_resolution_task` | **0**                                    |
| `risks_accepted_or_closed`      | **0**                                    |

#### 3. Distinciones obligatorias

| Concepto             | Regla                                                                                                                     |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| hallazgo             | evidencia técnica u operativa puntual; puede alimentar uno o varios riesgos                                               |
| riesgo consolidado   | posibilidad de efecto material con una identidad estable y una clase primaria                                             |
| severidad del riesgo | evaluación de impacto, probabilidad, detectabilidad, reversibilidad, alcance y controles                                  |
| prioridad `TREQ-*`   | urgencia de proteger una regla mediante prueba; no sustituye la severidad del riesgo                                      |
| deuda técnica        | condición de mantenibilidad o reproducibilidad sin efecto material actual demostrado; no puede ocultar un riesgo superior |
| control actual       | medida observada que reduce parcialmente exposición; no equivale a cierre sin prueba de eficacia                          |
| riesgo residual      | exposición que permanece después de controles probados; se evaluará durante arquitectura y transición                     |
| aceptación           | decisión futura excepcional con autoridad, vencimiento y evidencia; esta tarea no acepta riesgos                          |

#### 4. Criterios de clasificación

##### 4.1. `CRÍTICO`

Se utiliza cuando existe al menos una de estas condiciones:

- ruta activa o plausible hacia privilegio, secreto, identidad, dinero, privacidad o integridad empresarial;
- efecto potencial irreversible o difícil de compensar;
- riesgo sistémico que cruza aplicaciones, dominios o consumidores;
- incapacidad material de detectar, reconstruir, conciliar o revertir un fallo crítico;
- ausencia de fuente o comando canónico en un proceso cuya falsedad pueda afectar personas, dinero, inventario, producción, cumplimiento o continuidad.

##### 4.2. `ALTO`

Se utiliza para una exposición material con impacto operativo, de seguridad, integridad, rendimiento o transición, pero con alcance más acotado, control parcial verificable o reversibilidad razonable. Bloquea la decisión o transición relacionada, aunque no exige tratar toda la plataforma como comprometida.

##### 4.3. `MEDIO`

Se utiliza cuando el efecto es localizado, detectable y reversible, no existe evidencia de compromiso actual y la condición puede corregirse dentro del paquete relacionado sin bloquear toda la arquitectura.

##### 4.4. `DEUDA_TÉCNICA`

Se utiliza únicamente cuando la condición aumenta costo, complejidad, drift o probabilidad futura, pero no demuestra por sí sola un efecto material vigente. Si la misma condición habilita una exposición de seguridad, pérdida de datos o efecto empresarial, conserva la clase superior.

#### 5. Reglas de evaluación

1. Cada riesgo tiene una sola clase primaria.
2. La severidad no se calcula por cantidad de filas, objetos o TREQ, sino por el peor efecto plausible sustentado.
3. Un workaround manual, ausencia de incidentes o control parcial no reduce severidad sin prueba de eficacia y cobertura.
4. Un hallazgo puede participar en varios riesgos solo cuando protege efectos materialmente distintos; no se duplica para inflar conteos.
5. La clase refleja el estado actual observado. Arquitectura y transición deberán recalcular el riesgo residual después de controles implementados.
6. Todo cambio de clase exige evidencia nueva, tarea responsable y actualización del registro.
7. Ningún riesgo se considera aceptado, mitigado, resuelto o cerrado en esta tarea.
8. Los riesgos críticos bloquean la expansión de su superficie y los cambios irreversibles relacionados hasta superar su gate.

#### 6. Registro consolidado de riesgos

| Riesgo         | Clase           | Dominio                   | Declaración consolidada                                                                                                                                                         | Evidencia de origen                                    | Superficie afectada                                                                               | Impacto principal                                                                                       | Probabilidad | Detectabilidad | Controles actuales observados                                                                                | Resolución documental                                                      | Gate obligatorio                                                        | Requisitos relacionados                                                    |
| -------------- | --------------- | ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ------------ | -------------- | ------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------- | ----------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| `RSK-SUPA-001` | `CRÍTICO`       | `AUTORIZACIÓN`            | La aceptación de invitaciones puede materializar rol y sede aportados por el usuario sin una invitación vinculante validada.                                                    | SUPA-AUD-014; SUPA-AUD-015                             | staff-invitations-accept; staff_invitations; users; employees                                     | escalamiento de privilegios; acceso territorial indebido; identidad laboral falsa                       | ALTA         | MEDIA          | autenticación previa y tablas de invitación existentes, insuficientes para vincular autoridad                | SUPA-ARC-008; SUPA-ARC-009; SUPA-ARC-014; SUPA-ARC-015; SUPA-ARC-020       | CONTENCIÓN CRÍTICA antes de ampliar altas o permisos                    | TREQ-SUPABASE-190; TREQ-SUPABASE-191; TREQ-SUPABASE-507                    |
| `RSK-SUPA-002` | `CRÍTICO`       | `SECRETOS`                | Existe un secreto interno almacenado en public.app_config con lectura observada para anon y authenticated.                                                                      | SUPA-AUD-014; SUPA-AUD-015                             | public.app_config; políticas SELECT; consumidores internos                                        | exposición de credencial; invocación no autorizada; compromiso transversal                              | ALTA         | ALTA           | el esquema net no está expuesto actualmente y el valor no se reprodujo en documentación                      | SUPA-ARC-006; SUPA-ARC-015; SUPA-ARC-020; SUPA-TRANS-001                   | CONTENCIÓN CRÍTICA y prueba de revocación antes de cualquier transición | TREQ-SUPABASE-192; TREQ-SUPABASE-217; TREQ-SUPABASE-505                    |
| `RSK-SUPA-003` | `CRÍTICO`       | `SECRETOS`                | Bearers, claves de cron y secretos internos permanecen como literales o texto plano sin registro y rotación unificados.                                                         | SUPA-AUD-014; SUPA-AUD-015                             | cron.job.command; internal_job_secrets; variables Edge; credenciales externas                     | compromiso persistente en catálogos, respaldos o despliegues; revocación incompleta                     | ALTA         | MEDIA          | restricciones parciales por service_role y secretos separados por función                                    | SUPA-ARC-006; SUPA-ARC-020; SUPA-ARC-024; SUPA-TRANS-010                   | CONTENCIÓN CRÍTICA; inventario y rotación verificable                   | TREQ-SUPABASE-193; TREQ-SUPABASE-218; TREQ-SUPABASE-503; TREQ-SUPABASE-505 |
| `RSK-SUPA-004` | `CRÍTICO`       | `SEGURIDAD DE DATOS`      | Data API, grants, RLS, vistas y funciones privilegiadas no forman todavía una frontera uniforme de autorización por efecto empresarial.                                         | SUPA-AUD-003; SUPA-AUD-007; SUPA-AUD-009; SUPA-AUD-022 | schemas expuestos; 379 relaciones Vento; funciones SECURITY DEFINER; vistas; roles                | BOLA o IDOR; bypass RLS; lectura o mutación lateral; autoridad técnica excesiva                         | MEDIA        | BAJA           | RLS y grants existen en parte; app_private permanece no expuesto                                             | SUPA-ARC-005; SUPA-ARC-006; SUPA-ARC-014; SUPA-ARC-015; SUPA-ARC-016       | GATE DE SEGURIDAD antes de ampliar Data API, RPC o vistas               | TREQ-SUPABASE-427; TREQ-SUPABASE-449; TREQ-SUPABASE-468; TREQ-SUPABASE-507 |
| `RSK-SUPA-005` | `CRÍTICO`       | `PRIVACIDAD E IDENTIDAD`  | Las rutas de eliminación de cuenta no prueban de forma uniforme identidad objetivo, reautenticación server-side, exclusión mutua y estado único.                                | SUPA-AUD-014; SUPA-AUD-015; SUPA-AUD-023               | request-account-deletion; account-deletion; process-account-deletions; datos PASS                 | eliminación de otra cuenta; doble procesamiento; anonimización incompleta; incumplimiento de privacidad | ALTA         | MEDIA          | autenticación y worker existentes; tablas de solicitud permiten seguimiento parcial                          | SUPA-ARC-008; SUPA-ARC-010; SUPA-ARC-020; SUPA-ARC-022                     | CONTENCIÓN CRÍTICA y contrato único antes de operación ampliada         | TREQ-SUPABASE-202; TREQ-SUPABASE-233; TREQ-SUPABASE-234; TREQ-SUPABASE-508 |
| `RSK-SUPA-006` | `CRÍTICO`       | `PEDIDOS Y DINERO`        | Pedido, checkout, pago, webhook y registro de evidencia pueden confirmar efectos en operaciones separadas o fuentes competidoras.                                               | SUPA-AUD-014; SUPA-AUD-019; SUPA-AUD-022; SUPA-AUD-023 | public.orders; payments.transactions; pass y club; checkout RPC; payments-webhook                 | doble cobro; pago aplicado sin evidencia; pedido y saldo divergentes; reversión incoherente             | ALTA         | MEDIA          | idempotencia parcial y registros de transacción existentes                                                   | SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-017; SUPA-ARC-020; SUPA-TRANS-008     | GATE TRANSACCIONAL antes de sustituir flujos de venta o pago            | TREQ-SUPABASE-201; TREQ-SUPABASE-446; TREQ-SUPABASE-509; TREQ-SUPABASE-510 |
| `RSK-SUPA-007` | `CRÍTICO`       | `DRIFT Y CAMBIO`          | Existen DDL fuera de banda y objetos sin procedencia migratoria, sin gate que bloquee releases ante drift crítico.                                                              | SUPA-AUD-016; SUPA-AUD-017                             | migraciones; funciones; triggers; políticas; columnas; índices; configuración hosted              | entornos irreproducibles; cambio no revisado; rollback incierto; pérdida de trazabilidad                | ALTA         | BAJA           | historia remota y archivos Git permiten reconciliación parcial                                               | SUPA-ARC-024; SUPA-TRANS-001; SUPA-TRANS-013; SUPA-TRANS-015; SHELL-CI-017 | GATE DE DRIFT bloqueante antes de toda migración futura                 | TREQ-SUPABASE-275; TREQ-SUPABASE-290; TREQ-SUPABASE-516; TREQ-SUPABASE-528 |
| `RSK-SUPA-008` | `CRÍTICO`       | `RECUPERACIÓN`            | La historia de migraciones no demuestra replay clean-room, rollback por paquete, idempotencia ni reconstrucción integral de recursos administrados.                             | SUPA-AUD-016; SUPA-AUD-017; SUPA-AUD-020               | 549 versiones remotas; snapshots; Storage; Realtime; cron; Auth hosted                            | imposibilidad de reconstruir o revertir; pérdida prolongada de servicio; recuperación parcial           | MEDIA        | BAJA           | inventarios y fingerprints puntuales reducen incertidumbre, pero no prueban replay                           | SUPA-ARC-022; SUPA-ARC-024; SUPA-TRANS-009; SUPA-TRANS-011; SUPA-TRANS-013 | GATE DE RECUPERACIÓN antes de cutover o retiro                          | TREQ-SUPABASE-249; TREQ-SUPABASE-250; TREQ-SUPABASE-251; TREQ-SUPABASE-517 |
| `RSK-SUPA-009` | `CRÍTICO`       | `EVENTOS`                 | Los 395 eventos empresariales aprobados carecen de outbox, transporte, replay, dead-letter y trazabilidad física canónica.                                                      | SUPA-AUD-013; SUPA-AUD-014; SUPA-AUD-023               | VPROC-0001 a VPROC-0069; aplicaciones propietarias y consumidoras                                 | consumidoras desincronizadas; efectos perdidos o duplicados; imposibilidad de reconstruir handoffs      | ALTA         | BAJA           | triggers, Realtime, Edge y cron cubren señales aisladas, no el contrato VPROC                                | SUPA-ARC-007; SUPA-ARC-019; SUPA-ARC-020; SUPA-TRANS-003; SUPA-TRANS-007   | GATE DE EVENTOS antes de coordinación asíncrona canónica                | TREQ-PROC-1041; TREQ-SUPABASE-471; TREQ-SUPABASE-512; TREQ-SUPABASE-525    |
| `RSK-SUPA-010` | `CRÍTICO`       | `AUTOMATIZACIÓN`          | Cron, pg_net, webhooks y workers pueden registrar encolado como éxito, duplicar trabajo o perder fallos sin cola durable, claim, retry y dead-letter comunes.                   | SUPA-AUD-014; SUPA-AUD-020; SUPA-AUD-023               | 7 cron; 24 Edge Functions; pg_net; workers de privacidad, pagos, soporte y turnos                 | falso cierre; doble efecto; trabajo perdido; operación silenciosamente inconsistente                    | ALTA         | BAJA           | algunas funciones restringen EXECUTE y existen logs parciales de cron y net                                  | SUPA-ARC-007; SUPA-ARC-020; SUPA-ARC-022; SUPA-TRANS-009                   | GATE ASÍNCRONO antes de automatizar efectos críticos                    | TREQ-SUPABASE-198; TREQ-SUPABASE-199; TREQ-SUPABASE-203; TREQ-SUPABASE-513 |
| `RSK-SUPA-011` | `CRÍTICO`       | `IDENTIDAD Y AUTORIDAD`   | Identidades, roles y asignaciones presentan fuentes competidoras, usuarios sin vínculo Auth y conceptos role distintos sin autoridad transversal consolidada.                   | SUPA-AUD-010; SUPA-AUD-019; SUPA-AUD-022               | auth.users; public.users; employees; roles; site_operational_roles; viso.site_operational_roles   | acceso incorrecto; persona equivocada; revocación incompleta; decisiones con contexto falso             | ALTA         | MEDIA          | FK, RLS y funciones de contexto cubren partes del modelo                                                     | SUPA-ARC-008; SUPA-ARC-009; SUPA-ARC-010; SUPA-ARC-016; SUPA-TRANS-005     | GATE DE IDENTIDAD antes de consolidar permisos y sesiones               | TREQ-AUTH-001; TREQ-SUPABASE-335; TREQ-SUPABASE-350; TREQ-SUPABASE-508     |
| `RSK-SUPA-012` | `CRÍTICO`       | `INVENTARIO`              | Stock por sede, ubicación, posición, UOM y aliases cuantitativos no convergen aún en una autoridad y unidad completamente reconciliadas.                                        | SUPA-AUD-018; SUPA-AUD-019; SUPA-AUD-023               | inventory_stock_by_site; inventory_stock_by_location; positions; UOM profiles; movements          | existencia falsa; consumo o traslado incorrecto; costo y producción erróneos; pérdida física            | ALTA         | MEDIA          | movimientos, constraints y funciones de conteo existen; mismatches críticos consultados fueron cero al corte | SUPA-ARC-003; SUPA-ARC-012; SUPA-ARC-016; SUPA-ARC-017; SUPA-TRANS-005     | GATE DE INVENTARIO antes de migrar stock o retirar aliases              | TREQ-SUPABASE-358; TREQ-SUPABASE-359; TREQ-SUPABASE-361; TREQ-SUPABASE-510 |
| `RSK-SUPA-013` | `CRÍTICO`       | `COBERTURA DE PROCESOS`   | Veinte procesos VPROC no tienen fuente canónica actual y no pueden demostrar estado, corrección, cierre ni evidencia integral.                                                  | SUPA-AUD-022; SUPA-AUD-023                             | 20 procesos NO_CANONICAL_SOURCE en gobierno, SST, activos, finanzas, continuidad y otros dominios | operación manual no trazable; cierre falso; capacidad empresarial ausente; decisiones sin evidencia     | ALTA         | MEDIA          | documentos, soporte y objetos auxiliares cubren fragmentos sin constituir fuente                             | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; paquetes E5 propietarios         | GATE DE COBERTURA por proceso antes de declararlo digital u operativo   | TREQ-SUPABASE-460; TREQ-SUPABASE-484; TREQ-SUPABASE-511; TREQ-SUPABASE-525 |
| `RSK-SUPA-014` | `CRÍTICO`       | `COMANDOS`                | Veinticuatro procesos carecen de comando o RPC canónica y pueden depender de escrituras directas, helpers parciales o ejecución manual sin transición e idempotencia uniformes. | SUPA-AUD-006; SUPA-AUD-007; SUPA-AUD-023               | 24 procesos NO_CANONICAL_COMMAND_RPC; clientes Supabase; funciones y tablas de estado             | estados imposibles; bypass de autorización; doble efecto; reglas dispersas entre clientes               | ALTA         | BAJA           | RLS, triggers y helpers protegen casos aislados                                                              | SUPA-ARC-013; SUPA-ARC-014; SUPA-ARC-015; SUPA-ARC-016; SUPA-ARC-017       | GATE DE COMANDO antes de habilitar mutaciones canónicas                 | TREQ-SUPABASE-465; TREQ-SUPABASE-467; TREQ-SUPABASE-476; TREQ-SUPABASE-511 |
| `RSK-SUPA-015` | `CRÍTICO`       | `STORAGE Y PRIVACIDAD`    | Buckets, rutas, políticas, documentos y ciclos de vida no están gobernados de forma uniforme para sensibilidad, acceso, retención, borrado y orfandad.                          | SUPA-AUD-012; SUPA-AUD-014; SUPA-AUD-020; SUPA-AUD-022 | 14 buckets; 1101 objetos al corte; documentos laborales, candidatos, productos y entregas         | exposición de archivos; borrado prematuro o ausente; huérfanos; incumplimiento de privacidad            | MEDIA        | BAJA           | políticas Storage y buckets privados existen de forma parcial                                                | SUPA-ARC-018; SUPA-ARC-022; SUPA-TRANS-005; SUPA-TRANS-009                 | GATE DE STORAGE antes de mover, publicar o retirar objetos              | TREQ-SUPABASE-157; TREQ-SUPABASE-158; TREQ-SUPABASE-515; TREQ-SUPABASE-521 |
| `RSK-SUPA-016` | `CRÍTICO`       | `FUENTES DE VERDAD`       | Procesos y capacidades cruzan schemas y aplicaciones con objetos compartidos, fachadas y escritores actuales que no siempre coinciden con la propietaria aprobada.              | SUPA-AUD-019; SUPA-AUD-022; SUPA-AUD-023               | 379 relaciones; 69 procesos; PASS, PULSO, NEXO, FOGO, ORIGO, NUMERA, VISO, ANIMA y AURA           | doble fuente; corrección en lugar equivocado; consumidor que escribe; transición destructiva            | ALTA         | BAJA           | mapas 022 y 023 identifican objeto, capacidad, propietaria y consumidoras                                    | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-017; SUPA-TRANS-001     | GATE DE PROPIEDAD antes de decidir movimientos o fusiones               | TREQ-SUPABASE-433; TREQ-SUPABASE-446; TREQ-SUPABASE-463; TREQ-SUPABASE-510 |
| `RSK-SUPA-017` | `ALTO`          | `ARQUITECTURA`            | public concentra múltiples capacidades, compatibilidad, helpers, datos y superficies expuestas sin representar un dominio empresarial estable.                                  | SUPA-AUD-002; SUPA-AUD-003; SUPA-AUD-022               | schema public y sus 246 relaciones Vento observadas                                               | fronteras difusas; exposición accidental; dependencia de nombres físicos; migración compleja            | ALTA         | MEDIA          | inventario por objeto y clasificación de schemas disponibles                                                 | SUPA-ARC-001; SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-004; SUPA-ARC-005       | DECISIÓN ARQUITECTÓNICA previa a movimientos                            | TREQ-SUPABASE-424; TREQ-SUPABASE-431; TREQ-SUPABASE-526                    |
| `RSK-SUPA-018` | `ALTO`          | `AISLAMIENTO DE PRODUCTO` | Cincuenta y cuatro tablas VITAL comparten proyecto, configuración y ciclo de cambios con Vento OS sin una frontera de producto aprobada.                                        | SUPA-AUD-002; SUPA-AUD-021; SUPA-AUD-022               | schema vital; tipos; Auth; migraciones; recursos del proyecto                                     | acoplamiento de despliegues; exposición o tipos indebidos; mayor radio de impacto                       | MEDIA        | MEDIA          | VITAL está clasificado fuera de Vento OS en el mapa 022                                                      | SUPA-ARC-001; SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-023; SUPA-ARC-024       | DECISIÓN DE AISLAMIENTO antes de contratos y entornos objetivo          | TREQ-SUPABASE-445; TREQ-SUPABASE-524; TREQ-SUPABASE-526                    |
| `RSK-SUPA-019` | `ALTO`          | `REALTIME`                | Publicaciones y suscripciones no coinciden: existen consumidoras de tablas no publicadas, vistas no publicables y objetos publicados sin consumidor demostrado.                 | SUPA-AUD-013; SUPA-AUD-022                             | 2 publicaciones; 6 tablas empresariales publicadas; PASS, PULSO y ANIMA                           | función aparentemente activa que no recibe cambios; replicación sin finalidad; fuga de columnas         | ALTA         | MEDIA          | algunas suscripciones operan y el slot administrado no mostró errores en la muestra                          | SUPA-ARC-005; SUPA-ARC-015; SUPA-ARC-019; SUPA-TRANS-003; SUPA-TRANS-007   | GATE DE PUBLICACIÓN-CONSUMIDOR antes de cambios Realtime                | TREQ-SUPABASE-170; TREQ-SUPABASE-172; TREQ-SUPABASE-514                    |
| `RSK-SUPA-020` | `ALTO`          | `EDGE Y DESPLIEGUE`       | Fuente, config.toml, verify_jwt y bundle desplegado no tienen manifiesto común para las 24 Edge Functions.                                                                      | SUPA-AUD-014; SUPA-AUD-015; SUPA-AUD-016; SUPA-AUD-017 | 24 Edge Functions; rutas fuente; configuración hosted; bundles                                    | despliegue no reproducible; función pública inesperada; rollback y auditoría ambiguos                   | ALTA         | BAJA           | inventario remoto y búsquedas de fuente identifican diferencias concretas                                    | SUPA-ARC-020; SUPA-ARC-024; SUPA-TRANS-013; SUPA-TRANS-015                 | GATE DE MANIFIESTO antes de desplegar Edge                              | TREQ-SUPABASE-209; TREQ-SUPABASE-236; TREQ-SUPABASE-259; TREQ-SUPABASE-523 |
| `RSK-SUPA-021` | `ALTO`          | `TIPOS Y CONTRATOS`       | No existe artefacto Database generado, paquete compartido ni clientes tipados para tablas, vistas, RPC, enums y nullability.                                                    | SUPA-AUD-021                                           | 12 repositorios activos potenciales; 9 schemas Vento; 347 funciones; 62 vistas                    | drift de compilación y runtime; llamadas RPC incorrectas; campos o enums degradados                     | ALTA         | MEDIA          | TypeScript y SDK aportan tipos genéricos, no contrato de esquema                                             | SUPA-ARC-023; SUPA-TRANS-010; SUPA-TRANS-014; SHELL-PKG-001; SHELL-CI-017  | GATE DE TIPOS antes de promover cambios incompatibles                   | TREQ-SUPABASE-393; TREQ-SUPABASE-399; TREQ-SUPABASE-522                    |
| `RSK-SUPA-022` | `ALTO`          | `LEGACY`                  | Objetos, aliases, overloads y fachadas legacy siguen consumidos sin ledger de retiro, equivalencia, aviso, paridad ni rollback.                                                 | SUPA-AUD-018; SUPA-AUD-019                             | role_capabilities; qty aliases; campos PASS; vistas fachada; RPC sobrecargadas                    | retiro rompe clientes; divergencia silenciosa; doble mantenimiento; datos sin reconciliar               | ALTA         | MEDIA          | algunos candidatos y consumidores están identificados individualmente                                        | SUPA-ARC-005; SUPA-ARC-013; SUPA-TRANS-006; SUPA-TRANS-007; SUPA-TRANS-012 | GATE DE DEPRECACIÓN antes de eliminar o renombrar                       | TREQ-SUPABASE-303; TREQ-SUPABASE-305; TREQ-SUPABASE-518                    |
| `RSK-SUPA-023` | `ALTO`          | `CALIDAD DE DATOS`        | Duplicados de proveedores y clientes, usuarios sin Auth, referencias lógicas sin FK y constraints no validadas requieren reconciliación antes de consolidar autoridades.        | SUPA-AUD-005; SUPA-AUD-019                             | proveedores; clientes; public.users; 47 columnas *_id; role_code; invitaciones                    | merge incorrecto; identidad equivocada; relación huérfana; restricciones que fallan al validar          | ALTA         | MEDIA          | consultas puntuales cuantifican casos y varias relaciones críticas no mostraron huérfanos                    | SUPA-ARC-012; SUPA-TRANS-005; SUPA-TRANS-009; SUPA-TRANS-011               | GATE DE CALIDAD antes de constraints, merges o backfills                | TREQ-SUPABASE-346; TREQ-SUPABASE-350; TREQ-SUPABASE-351; TREQ-SUPABASE-519 |
| `RSK-SUPA-024` | `ALTO`          | `ÍNDICES`                 | Existen índices duplicados y 444 de 795 FKs empresariales carecen de un índice válido con prefijo líder.                                                                        | SUPA-AUD-020                                           | attendance_logs; inventory_stock_by_site; claves foráneas empresariales                           | latencia y bloqueo crecientes; costo de escritura innecesario; degradación bajo volumen                 | ALTA         | ALTA           | conteos y definiciones exactas disponibles; no se autorizó DDL                                               | SUPA-ARC-021; SUPA-TRANS-009; SUPA-TRANS-010                               | GATE DE ÍNDICES basado en carga y planes antes de DDL                   | TREQ-SUPABASE-365; TREQ-SUPABASE-368; TREQ-SUPABASE-520                    |
| `RSK-SUPA-025` | `ALTO`          | `RENDIMIENTO`             | Consultas activas muestran Seq Scan, sorts adicionales y no existe gate de regresión de planes con fixtures representativos.                                                    | SUPA-AUD-020                                           | catálogo; asistencia; turnos; consultas críticas de aplicaciones                                  | degradación no detectada; timeout; saturación; cambios de plan tras crecimiento                         | MEDIA        | BAJA           | EXPLAIN puntual y pg_stat_statements proporcionan evidencia parcial                                          | SUPA-ARC-021; SUPA-TRANS-009; SUPA-TRANS-010; SHELL-CI-017                 | GATE DE PLANES antes de promover migraciones e índices                  | TREQ-SUPABASE-371; TREQ-SUPABASE-376; TREQ-SUPABASE-520                    |
| `RSK-SUPA-026` | `ALTO`          | `CRECIMIENTO Y RETENCIÓN` | No hay series históricas ni umbrales aprobados de crecimiento, partición, archivado y purga para tablas, WAL y registros técnicos.                                              | SUPA-AUD-012; SUPA-AUD-020                             | eventos; movimientos; auditoría; webhooks; cron.job_run_details; delivery_quotes                  | crecimiento indefinido; backups lentos; consultas degradadas; retención contraria a finalidad           | ALTA         | MEDIA          | corte puntual de tamaños y edades disponible                                                                 | SUPA-ARC-021; SUPA-ARC-022; SUPA-TRANS-009; SUPA-TRANS-010                 | GATE DE RETENCIÓN antes de operación prolongada y archivo               | TREQ-SUPABASE-381; TREQ-SUPABASE-383; TREQ-SUPABASE-521                    |
| `RSK-SUPA-027` | `ALTO`          | `STORAGE`                 | product-images concentra aproximadamente 378,6 MB en 45 objetos sin política probada de compresión, deduplicación y versiones.                                                  | SUPA-AUD-012; SUPA-AUD-020                             | bucket product-images y consumidores de catálogo                                                  | costo y transferencia excesivos; versiones huérfanas; experiencia lenta                                 | MEDIA        | ALTA           | inventario de objetos y tamaño permite medir la línea base                                                   | SUPA-ARC-018; SUPA-ARC-021; SUPA-ARC-022; SUPA-TRANS-010                   | GATE DE CICLO DE VIDA antes de ampliar contenido                        | TREQ-SUPABASE-389; TREQ-SUPABASE-515; TREQ-SUPABASE-521                    |
| `RSK-SUPA-028` | `ALTO`          | `OBSERVABILIDAD`          | No existe trazabilidad unificada entre cron run, pg_net request, Edge invocation, evento, efecto empresarial, retry y alerta.                                                   | SUPA-AUD-014; SUPA-AUD-017; SUPA-AUD-023               | cron; pg_net; Edge; procesos VPROC; soporte y pagos                                               | incidente invisible; falso éxito; investigación lenta; reconciliación manual                            | ALTA         | BAJA           | logs separados y request_id parciales permiten diagnóstico puntual                                           | SUPA-ARC-007; SUPA-ARC-020; SUPA-ARC-021; SUPA-TRANS-009                   | GATE DE OBSERVABILIDAD para automatismos críticos                       | TREQ-SUPABASE-210; TREQ-SUPABASE-478; TREQ-SUPABASE-526                    |
| `RSK-SUPA-029` | `ALTO`          | `ENTORNOS`                | Local, preview, staging, producción, branches y configuración hosted no tienen overlays certificados ni inventario común.                                                       | SUPA-AUD-015; SUPA-AUD-016                             | config.toml; Auth hosted; network restrictions; branches; secretos; funciones                     | cambio correcto en un entorno y peligroso en otro; mezcla de datos o credenciales                       | MEDIA        | BAJA           | proyecto dev identificado y algunas diferencias config-remoto inventariadas                                  | SUPA-ARC-024; SUPA-TRANS-013; SUPA-TRANS-015                               | GATE DE ENTORNO antes de promoción                                      | TREQ-SUPABASE-239; TREQ-SUPABASE-258; TREQ-SUPABASE-523                    |
| `RSK-SUPA-030` | `ALTO`          | `CREDENCIALES EXTERNAS`   | Credenciales de Google, Maps, Resend, Wompi, Expo y workers carecen de matriz transversal de propietario, alcance, ambiente y rotación.                                         | SUPA-AUD-015                                           | 24 Edge Functions; proveedores externos; apps móviles y web                                       | privilegio excesivo; revocación incompleta; indisponibilidad durante rotación                           | MEDIA        | BAJA           | variables separadas por despliegue y algunas restricciones por proveedor                                     | SUPA-ARC-020; SUPA-ARC-024; SUPA-TRANS-010                                 | GATE DE CREDENCIALES antes de producción                                | TREQ-SUPABASE-226; TREQ-SUPABASE-227; TREQ-SUPABASE-503; TREQ-SUPABASE-523 |
| `RSK-SUPA-031` | `ALTO`          | `REALTIME`                | Reconexión, duplicado, orden, replay, row filters y capacidad no tienen semántica común entre consumidoras.                                                                     | SUPA-AUD-013; SUPA-AUD-020                             | PASS, PULSO, ANIMA; canales Realtime; payload.old                                                 | eventos perdidos o repetidos; recargas masivas; datos fuera de orden; carga amplificada                 | ALTA         | MEDIA          | SDK maneja conexiones básicas y clientes filtran parte del payload                                           | SUPA-ARC-019; SUPA-ARC-021; SUPA-TRANS-009; SUPA-TRANS-010                 | GATE DE RESILIENCIA Realtime antes de dependencia crítica               | TREQ-SUPABASE-176; TREQ-SUPABASE-180; TREQ-SUPABASE-185; TREQ-SUPABASE-514 |
| `RSK-SUPA-032` | `ALTO`          | `AUTH`                    | Usuarios, sesiones, revocación y vínculos trabajador-cliente-dispositivo no están consolidados en un ciclo canónico verificable.                                                | SUPA-AUD-010; SUPA-AUD-015; SUPA-AUD-022               | auth.users; public.users; employees; dispositivos; PASS; sesiones                                 | sesión vigente tras retiro; identidad cruzada; datos de sujeto equivocado; revocación incompleta        | MEDIA        | BAJA           | Supabase Auth, vínculos parciales y funciones de contexto existen                                            | SUPA-ARC-008; SUPA-ARC-009; SUPA-ARC-010; SUPA-TRANS-005                   | GATE DE CICLO DE IDENTIDAD antes de integración transversal             | TREQ-SUPABASE-508; TREQ-AUTH-002; TREQ-AUTH-006                            |
| `RSK-SUPA-033` | `ALTO`          | `GOBIERNO DE OBJETOS`     | Objetos carecen de comentarios, propietario empresarial, escritor y proyección declarados de forma ejecutable y recurrente.                                                     | SUPA-AUD-004; SUPA-AUD-017; SUPA-AUD-019; SUPA-AUD-022 | 379 relaciones Vento; funciones; triggers; políticas; Storage y automatismos                      | cambio sobre objeto equivocado; retiro inseguro; ownership inferido desde schema o rol PostgreSQL       | ALTA         | BAJA           | mapa 022 documenta propiedad preliminar y consumidores                                                       | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-011; SUPA-ARC-016; SHELL-CI-017       | GATE DE CATÁLOGO antes de transición por objeto                         | TREQ-SUPABASE-333; TREQ-SUPABASE-424; TREQ-SUPABASE-526                    |
| `RSK-SUPA-034` | `ALTO`          | `TRIGGERS`                | Orden, solapamiento, duplicidad y funciones trigger sin trigger asociado no tienen contrato semántico ni gate recurrente.                                                       | SUPA-AUD-008; SUPA-AUD-017; SUPA-AUD-022               | 196 triggers Vento; employee_push_tokens; dos trigger functions sin asociación                    | efecto doble, orden incorrecto, automatismo muerto o drift de despliegue                                | MEDIA        | MEDIA          | inventario exacto de trigger, evento, orden y función disponible                                             | SUPA-ARC-013; SUPA-ARC-017; SUPA-TRANS-003; SUPA-TRANS-009                 | GATE DE AUTOMATISMOS antes de modificar tablas afectadas                | TREQ-SUPABASE-112; TREQ-SUPABASE-294; TREQ-SUPABASE-513                    |
| `RSK-SUPA-035` | `ALTO`          | `CONSUMIDORES`            | Búsqueda SQL y Git no certifica todos los consumidores externos, manuales, no indexados o desplegados de objetos legacy y Data API.                                             | SUPA-AUD-018; SUPA-AUD-021; SUPA-AUD-022               | 12 repositorios activos; plataformas externas; scripts; clientes no indexados                     | retiro rompe consumidor oculto; exposición sin finalidad; contrato incompleto                           | MEDIA        | BAJA           | búsquedas de código y mapas de consumidoras cubren evidencia localizada                                      | SUPA-ARC-016; SUPA-TRANS-003; SUPA-TRANS-007; SUPA-TRANS-012               | GATE DE CONSUMIDORES antes de retirar o cerrar compatibilidad           | TREQ-SUPABASE-305; TREQ-SUPABASE-436; TREQ-SUPABASE-518                    |
| `RSK-SUPA-036` | `ALTO`          | `RPC`                     | Overloads, aliases y helpers se identifican por nombre sin un registro ejecutable de firma, rol, efecto, seguridad y proceso.                                                   | SUPA-AUD-006; SUPA-AUD-007; SUPA-AUD-018; SUPA-AUD-023 | 347 funciones Vento; RPC checkout, producción, inventario y autorización                          | resolución ambigua; cliente incompatible; helper tratado como comando; bypass de reglas                 | ALTA         | MEDIA          | inventario de firmas y clasificación preliminar de funciones disponible                                      | SUPA-ARC-013; SUPA-ARC-014; SUPA-ARC-016; SUPA-TRANS-003; SUPA-TRANS-014   | GATE DE FIRMA antes de publicar o retirar RPC                           | TREQ-SUPABASE-319; TREQ-SUPABASE-408; TREQ-SUPABASE-465; TREQ-SUPABASE-522 |
| `RSK-SUPA-037` | `MEDIO`         | `REDIRECCIÓN DE PAGOS`    | payments-return recibe identificadores por query string que solo pueden usarse como pistas y requieren revalidación server-side.                                                | SUPA-AUD-014                                           | payments-return; deep links PASS                                                                  | presentación de estado equivocado o navegación engañosa sin efecto directo si el servidor revalida      | MEDIA        | ALTA           | la aplicación puede consultar el estado canónico antes de confirmar                                          | SUPA-ARC-020; SUPA-TRANS-009                                               | PRUEBA NEGATIVA obligatoria en el paquete de pagos                      | TREQ-SUPABASE-207; TREQ-SUPABASE-509                                       |
| `RSK-SUPA-038` | `MEDIO`         | `METADATOS DE DESPLIEGUE` | Metadatos de despliegue mezclan rutas temporales y absolutas de equipos sin manifiesto único de fuente, versión y SHA.                                                          | SUPA-AUD-014; SUPA-AUD-016                             | bundles Edge y registros de despliegue                                                            | procedencia ambigua; filtración menor de estructura local; diagnóstico inconsistente                    | MEDIA        | ALTA           | Git y metadata remota permiten correlación manual                                                            | SUPA-ARC-020; SUPA-ARC-024; SUPA-TRANS-013                                 | NORMALIZACIÓN antes de certificar despliegues                           | TREQ-SUPABASE-209; TREQ-SUPABASE-260                                       |
| `RSK-SUPA-039` | `MEDIO`         | `EXTENSIONES`             | Extensiones en public o sin uso empresarial demostrado carecen de decisión explícita de ubicación y ciclo de vida.                                                              | SUPA-AUD-015                                           | unaccent; uuid-ossp; schema extensions                                                            | namespace contaminado; dependencia implícita; superficie innecesaria                                    | BAJA         | ALTA           | inventario de extensiones y referencias disponible                                                           | SUPA-ARC-001; SUPA-ARC-006; SUPA-ARC-011                                   | DECISIÓN ARQUITECTÓNICA antes de mover o retirar                        | TREQ-SUPABASE-212; TREQ-SUPABASE-213                                       |
| `RSK-SUPA-040` | `MEDIO`         | `CANALES`                 | Nombres de canales, Broadcast y Presence carecen de catálogo, convención y política común.                                                                                      | SUPA-AUD-013                                           | 17 nombres locales; Broadcast; Presence                                                           | colisiones; suscripción equivocada; mantenimiento y depuración difíciles                                | MEDIA        | ALTA           | nombres actuales están localizados en código                                                                 | SUPA-ARC-011; SUPA-ARC-019; SUPA-TRANS-014                                 | NORMALIZACIÓN antes de ampliar canales                                  | TREQ-SUPABASE-177; TREQ-SUPABASE-183                                       |
| `RSK-SUPA-041` | `MEDIO`         | `CLASIFICACIÓN DE CICLO`  | Tablas vacías o con cero consumidores pueden ser capacidad futura, estructura abandonada o compatibilidad y no se pueden retirar por conteo.                                    | SUPA-AUD-018; SUPA-AUD-022                             | tablas vacías; asset_maintenance_records; plantillas; vistas sin dependientes SQL                 | retiro prematuro o mantenimiento indefinido de objetos innecesarios                                     | MEDIA        | ALTA           | estado y candidatos están inventariados sin decidir destino                                                  | SUPA-ARC-003; SUPA-TRANS-001; SUPA-TRANS-002; SUPA-TRANS-012               | CLASIFICACIÓN antes de cualquier retiro                                 | TREQ-SUPABASE-324; TREQ-SUPABASE-518                                       |
| `RSK-SUPA-042` | `MEDIO`         | `POLÍTICAS DUPLICADAS`    | Siete grupos de políticas RLS semánticamente equivalentes duplicadas aumentan complejidad sin conflicto material comprobado.                                                    | SUPA-AUD-019                                           | políticas RLS duplicadas                                                                          | mantenimiento divergente; revisión difícil; riesgo futuro de semánticas diferentes                      | MEDIA        | ALTA           | equivalencia actual fue identificada                                                                         | SUPA-ARC-015; SUPA-TRANS-002; SUPA-TRANS-009                               | CONSOLIDACIÓN controlada con pruebas de paridad                         | TREQ-SUPABASE-342; TREQ-SUPABASE-343                                       |
| `RSK-SUPA-043` | `MEDIO`         | `SEMÁNTICA LEGACY`        | Aliases cuantitativos, NULL como comportamiento y campos duales conservan paridad actual pero semántica implícita.                                                              | SUPA-AUD-018; SUPA-AUD-019                             | qty; qty_base; flags nullable; invitation role y site duales                                      | interpretación diferente entre clientes; migración ambigua; errores al completar defaults               | MEDIA        | MEDIA          | consultas de paridad y consumidores actuales están localizados                                               | SUPA-ARC-012; SUPA-TRANS-005; SUPA-TRANS-006; SUPA-TRANS-014               | CONTRATO DE COMPATIBILIDAD antes de backfill                            | TREQ-SUPABASE-320; TREQ-SUPABASE-353; TREQ-SUPABASE-518                    |
| `RSK-SUPA-044` | `MEDIO`         | `TIEMPO Y CRON`           | Nombres y horarios UTC de jobs de asistencia inducen una semántica local de Bogotá incorrecta aunque la ejecución sea observable.                                               | SUPA-AUD-014                                           | close_open_attendance_day_end; nombres de cron                                                    | cierre en momento no esperado; operación confusa; corrección manual                                     | MEDIA        | ALTA           | horarios reales y zona fueron reconciliados documentalmente                                                  | SUPA-ARC-011; SUPA-ARC-020; SUPA-TRANS-009                                 | PRUEBA TEMPORAL antes de cambiar agenda                                 | TREQ-SUPABASE-196; TREQ-SUPABASE-197                                       |
| `RSK-SUPA-045` | `MEDIO`         | `CAPACIDAD`               | Superficies de menor volumen carecen de baseline y prueba de capacidad, aunque no mostraron saturación en la muestra.                                                           | SUPA-AUD-013; SUPA-AUD-020                             | Realtime; funciones; Storage; consultas administrativas                                           | degradación futura no detectada; dimensionamiento tardío                                                | MEDIA        | MEDIA          | métricas puntuales y tamaños actuales disponibles                                                            | SUPA-ARC-021; SUPA-TRANS-010                                               | BASELINE antes de declarar readiness                                    | TREQ-SUPABASE-185; TREQ-SUPABASE-520                                       |
| `RSK-SUPA-046` | `MEDIO`         | `AUTOMATISMOS HUÉRFANOS`  | Dos funciones de trigger no tienen trigger asociado y requieren confirmar si son residuo, reserva o despliegue incompleto.                                                      | SUPA-AUD-008; SUPA-AUD-022                             | funciones trigger sin asociación                                                                  | código muerto; expectativa falsa de automatización; drift localizado                                    | MEDIA        | ALTA           | inventario de dependencias permite localizarlas                                                              | SUPA-ARC-013; SUPA-TRANS-001; SUPA-TRANS-002                               | CLASIFICACIÓN antes de retiro o asociación                              | TREQ-SUPABASE-429; TREQ-SUPABASE-513                                       |
| `RSK-SUPA-047` | `DEUDA_TÉCNICA` | `DOCUMENTACIÓN`           | Comentarios, descripción de propósito y propiedad técnica no están completos y obligan a reconstruir contexto desde nombres y código.                                           | SUPA-AUD-001; SUPA-AUD-004; SUPA-AUD-017               | schemas, tablas, funciones y configuración                                                        | mayor costo de revisión; errores de mantenimiento; onboarding lento                                     | ALTA         | ALTA           | inventarios E3 conservan contexto externo al catálogo                                                        | SUPA-ARC-011; SUPA-TRANS-014                                               | DEUDA PROGRAMADA; no sustituye riesgos de ownership                     | TREQ-SUPABASE-291; TREQ-SUPABASE-526                                       |
| `RSK-SUPA-048` | `DEUDA_TÉCNICA` | `VALIDADORES`             | Inventarios, mapas y clasificaciones dependen de consultas puntuales y no de validadores ejecutables recurrentes.                                                               | SUPA-AUD-013 a SUPA-AUD-023                            | documentación E3; catálogos; CI                                                                   | drift documental; conteos obsoletos; regresiones no bloqueadas                                          | ALTA         | MEDIA          | hashes y scripts parciales permiten validación manual                                                        | SHELL-CI-017; SUPA-TRANS-013; SUPA-TRANS-015                               | DEUDA BLOQUEANTE para cierre automatizado de arquitectura               | TREQ-SUPABASE-187; TREQ-SUPABASE-302; TREQ-SUPABASE-528                    |
| `RSK-SUPA-049` | `DEUDA_TÉCNICA` | `BASELINES`               | No existe serie histórica normalizada de rendimiento, tamaños, planes, WAL, Storage y crecimiento.                                                                              | SUPA-AUD-020                                           | base de datos y servicios administrados                                                           | decisiones reactivas; tendencia y regresión difíciles de demostrar                                      | ALTA         | MEDIA          | corte puntual de E3 establece punto inicial                                                                  | SUPA-ARC-021; SUPA-ARC-022; SUPA-TRANS-010                                 | DEUDA DE OBSERVABILIDAD con medición periódica                          | TREQ-SUPABASE-363; TREQ-SUPABASE-381; TREQ-SUPABASE-520                    |
| `RSK-SUPA-050` | `DEUDA_TÉCNICA` | `CLIENTES SDK`            | Fábricas Supabase duplicadas y versiones SDK divergentes elevan el costo de introducir contratos compartidos.                                                                   | SUPA-AUD-021                                           | repositorios de aplicaciones; clientes browser, server y mobile                                   | comportamiento desigual; upgrades fragmentados; errores repetidos                                       | ALTA         | ALTA           | inventario de versiones y clientes representativos disponible                                                | SHELL-PKG-001; SUPA-ARC-023; SUPA-TRANS-010; SUPA-TRANS-014                | DEUDA DE PLATAFORMA antes de adopción de tipos                          | TREQ-SUPABASE-403; TREQ-SUPABASE-411; TREQ-SUPABASE-522                    |
| `RSK-SUPA-051` | `DEUDA_TÉCNICA` | `ESCAPES DE TIPADO`       | any, casts y overrideTypes no tienen ledger, justificación, propietario ni presupuesto decreciente.                                                                             | SUPA-AUD-021                                           | TypeScript de aplicaciones y Edge                                                                 | drift oculto; refactor costoso; falso sentido de seguridad                                              | ALTA         | MEDIA          | compilación TypeScript detecta parte de los errores no silenciados                                           | SHELL-CI-017; SUPA-ARC-023; SUPA-TRANS-010                                 | DEUDA MEDIBLE mediante presupuesto y retiro                             | TREQ-SUPABASE-407; TREQ-SUPABASE-408; TREQ-SUPABASE-522                    |
| `RSK-SUPA-052` | `DEUDA_TÉCNICA` | `CONVENCIONES SQL`        | Funciones, search_path, nombres, updated_at y helpers repetidos carecen de convención única aplicada por lint.                                                                  | SUPA-AUD-006; SUPA-AUD-007; SUPA-AUD-008               | 347 funciones; 196 triggers; helpers de timestamps y autorización                                 | revisión lenta; inconsistencias; mayor probabilidad de error futuro                                     | ALTA         | ALTA           | inventarios de firma y seguridad permiten definir la regla                                                   | SUPA-ARC-011; SUPA-ARC-013; SUPA-ARC-014; SHELL-CI-017                     | DEUDA DE ESTANDARIZACIÓN antes de nuevas funciones                      | TREQ-SUPABASE-425; TREQ-SUPABASE-427; TREQ-SUPABASE-528                    |
| `RSK-SUPA-053` | `DEUDA_TÉCNICA` | `DEPRECACIÓN`             | No existe manifiesto transversal de alias, versión, fecha de aviso, consumidores, condición de salida y retiro.                                                                 | SUPA-AUD-018                                           | tablas, columnas, vistas, funciones, políticas y tipos legacy                                     | compatibilidad indefinida; decisiones de retiro repetidas; inventario obsoleto                          | ALTA         | ALTA           | candidatos legacy están identificados en E3                                                                  | SUPA-TRANS-006; SUPA-TRANS-007; SUPA-TRANS-012; SUPA-TRANS-014             | DEUDA CON FECHA DE SALIDA por objeto                                    | TREQ-SUPABASE-303; TREQ-SUPABASE-306; TREQ-SUPABASE-518                    |
| `RSK-SUPA-054` | `DEUDA_TÉCNICA` | `DECLARACIÓN DE RECURSOS` | Buckets, publicaciones Realtime y cron jobs no tienen manifiestos declarativos versionados junto a migraciones y configuración.                                                 | SUPA-AUD-016                                           | 14 buckets; 2 publicaciones; 7 cron jobs                                                          | drift de recursos administrados; reconstrucción manual; diferencias entre ambientes                     | ALTA         | MEDIA          | inventarios remotos completos disponibles                                                                    | SUPA-ARC-018; SUPA-ARC-019; SUPA-ARC-020; SUPA-ARC-024; SUPA-TRANS-015     | DEUDA DE INFRAESTRUCTURA COMO CÓDIGO                                    | TREQ-SUPABASE-264; TREQ-SUPABASE-265; TREQ-SUPABASE-266; TREQ-SUPABASE-523 |
| `RSK-SUPA-055` | `DEUDA_TÉCNICA` | `TRAZAS`                  | No existe una vista o herramienta única que una trace_context, correlation_id, request_id, evento y efecto empresarial.                                                         | SUPA-AUD-014; SUPA-AUD-023                             | aplicaciones; Edge; cron; Realtime; base de datos                                                 | diagnóstico manual; MTTR alto; evidencia dispersa                                                       | ALTA         | MEDIA          | algunos IDs y logs existen por flujo                                                                         | SUPA-ARC-007; SUPA-ARC-019; SUPA-ARC-020; SUPA-TRANS-009                   | DEUDA DE TELEMETRÍA; no sustituye controles transaccionales             | TREQ-SUPABASE-210; TREQ-SUPABASE-478; TREQ-SUPABASE-528                    |
| `RSK-SUPA-056` | `DEUDA_TÉCNICA` | `GOBIERNO DE RIESGO`      | No existe gate que reevalúe automáticamente severidad, evidencia, aceptación, vencimiento y cierre de riesgos al cambiar Supabase.                                              | SUPA-AUD-024                                           | registro de riesgos E3; cambios de schema, configuración y consumidores                           | clasificación obsoleta; riesgo aceptado sin expiración; cierre sin evidencia                            | ALTA         | MEDIA          | esta tarea crea la línea base documental y referencias TREQ                                                  | SHELL-CI-017; SUPA-ARC-024; SUPA-TRANS-013; SUPA-TRANS-015                 | DEUDA DE GOBIERNO con verificación recurrente                           | TREQ-SUPABASE-503; TREQ-SUPABASE-528; TREQ-SUPABASE-530                    |

#### 7. Resumen por clase

| Clase           | Cantidad | Regla de salida                                                                                              |
| --------------- | -------: | ------------------------------------------------------------------------------------------------------------ |
| `CRÍTICO`       |   **16** | contención y gate probado antes de ampliar superficie, ejecutar transición irreversible o declarar readiness |
| `ALTO`          |   **20** | decisión arquitectónica y prueba del paquete antes del cambio relacionado                                    |
| `MEDIO`         |   **10** | tratamiento dentro del paquete aplicable con evidencia de regresión y cierre                                 |
| `DEUDA_TÉCNICA` |   **10** | backlog con propietario, medición, fecha y criterio de retiro; no puede degradar riesgos superiores          |
| **Total**       |   **56** | ninguna fila sin destino                                                                                     |

#### 8. Matriz de exposición por proceso

La clase máxima del proceso representa la exposición más severa actualmente vinculada a su fuente, comando, eventos, propiedad o consumidoras. No modifica la criticidad empresarial del proceso ni la prioridad de implementación.

| Proceso                                                                                                                           | Propietaria | Cobertura física observada | Clase máxima | Semillas heredadas de SUPA-AUD-023                                                                                   | Riesgos primarios                                              | Destino principal                                                                     |
| --------------------------------------------------------------------------------------------------------------------------------- | ----------- | -------------------------- | ------------ | -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `VPROC-0001` — Gobernar decisiones empresariales con registro, alcance, responsable, compromisos y seguimiento                    | `viso`      | `NO_CANONICAL_SOURCE`      | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC                                                    | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-014`; `RSK-SUPA-011` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0002` — Mantener una estructura organizativa y jurídica coherente entre empresas, marcas, establecimientos, sedes y áreas  | `viso`      | `PARTIAL_CURRENT`          | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE                                                                         | `RSK-SUPA-009`; `RSK-SUPA-011`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0003` — Gobernar responsabilidades, políticas, delegaciones y límites de decisión mediante versiones vigentes              | `viso`      | `PARTIAL_CURRENT`          | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE                                                                         | `RSK-SUPA-009`; `RSK-SUPA-011`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0004` — Coordinar compromisos y transferencias de trabajo entre negocios, sedes y áreas                                    | `viso`      | `NO_CANONICAL_SOURCE`      | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC                                                    | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-014`; `RSK-SUPA-011` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0005` — Planear dotación y ejecutar selección sin mezclar necesidad laboral, candidato y trabajador activo                 | `viso`      | `FUTURE_OR_PARTIAL`        | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, DEFERRED_OR_FUTURE, PARTIAL_SOURCE, OWNER_IMPLEMENTATION_GAP                           | `RSK-SUPA-009`; `RSK-SUPA-039`; `RSK-SUPA-011`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; paquetes E5 de la propietaria               |
| `VPROC-0006` — Orquestar vinculación, expediente, incorporación, preparación y habilitación inicial de la persona                 | `viso`      | `PARTIAL_CURRENT`          | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE                                                                         | `RSK-SUPA-009`; `RSK-SUPA-011`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0007` — Administrar asignaciones laborales y programación publicada con historial y revisión controlada                    | `viso`      | `PARTIAL_CURRENT`          | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE                                                                         | `RSK-SUPA-009`; `RSK-SUPA-011`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0008` — Capturar asistencia como hechos inmutables y corregirla mediante decisiones auditables                             | `anima`     | `IMPLEMENTED_CURRENT`      | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT                                                                                         | `RSK-SUPA-009`; `RSK-SUPA-011`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0009` — Gestionar novedades, ausencias, permisos y reemplazos como casos laborales completos                               | `viso`      | `PARTIAL_CURRENT`          | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC                                               | `RSK-SUPA-009`; `RSK-SUPA-014`; `RSK-SUPA-011`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0010` — Preparar y reconciliar el paquete autorizado para pagos y beneficios laborales                                     | `numera`    | `NO_CANONICAL_SOURCE`      | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE                                                                              | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-006`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-017                                |
| `VPROC-0011` — Orquestar retiro laboral, devolución, revocación de accesos y cierre documental                                    | `viso`      | `FRAGMENTED_CURRENT`       | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE                                                                      | `RSK-SUPA-009`; `RSK-SUPA-016`; `RSK-SUPA-011`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0012` — Gestionar riesgos, inspecciones, controles preventivos y acciones correctivas                                      | `viso`      | `NO_CANONICAL_SOURCE`      | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC                                                    | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-014`; `RSK-SUPA-011` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0013` — Gestionar incidentes, accidentes y emergencias con respuesta inmediata y expediente posterior                      | `viso`      | `NO_CANONICAL_SOURCE`      | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC                                                    | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-014`; `RSK-SUPA-011` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0014` — Ejecutar controles de higiene, inocuidad y cumplimiento mediante procedimientos versionados                        | `viso`      | `NO_CANONICAL_SOURCE`      | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC                                                    | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-014`; `RSK-SUPA-011` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0015` — Gobernar el ciclo de vida de productos, presentaciones, unidades y equivalencias                                   | `nexo`      | `PARTIAL_CURRENT`          | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, CROSS_DOMAIN_BOUNDARY                                                  | `RSK-SUPA-009`; `RSK-SUPA-016`; `RSK-SUPA-012`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0016` — Gestionar desarrollo, prueba, aprobación, publicación y versión de recetas                                         | `fogo`      | `PARTIAL_CURRENT`          | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE                                                                         | `RSK-SUPA-009`; `RSK-SUPA-016`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0017` — Publicar oferta y disponibilidad desde una definición gobernada hacia todos los canales                            | `pulso`     | `FRAGMENTED_CURRENT`       | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, CROSS_DOMAIN_BOUNDARY                                               | `RSK-SUPA-009`; `RSK-SUPA-016`; `RSK-SUPA-006`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-017                                |
| `VPROC-0018` — Mantener especificaciones, alérgenos, restricciones y criterios de calidad del producto                            | `nexo`      | `PARTIAL_CURRENT`          | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE                                                                         | `RSK-SUPA-009`; `RSK-SUPA-012`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0019` — Capturar y priorizar necesidades de compra mediante una entrada única y trazable                                   | `origo`     | `PARTIAL_CURRENT`          | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE                                                                         | `RSK-SUPA-009`; `RSK-SUPA-016`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0020` — Comparar proveedores y condiciones con evidencia suficiente para decidir                                           | `origo`     | `PARTIAL_CURRENT`          | `MEDIO`      | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE                                                                         | `RSK-SUPA-009`; `RSK-SUPA-016`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0021` — Aprobar y emitir compras separando flujo ordinario, urgencia y excepción                                           | `origo`     | `PARTIAL_CURRENT`          | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC                                               | `RSK-SUPA-009`; `RSK-SUPA-014`; `RSK-SUPA-016`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0022` — Recibir compras, verificar conformidad y resolver diferencias sin separar recepción física, documental y económica | `origo`     | `FRAGMENTED_CURRENT`       | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, CROSS_DOMAIN_BOUNDARY                                               | `RSK-SUPA-009`; `RSK-SUPA-016`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0023` — Gobernar sedes, LOC, zonas, posiciones y condiciones de almacenamiento                                             | `nexo`      | `IMPLEMENTED_CURRENT`      | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT                                                                                         | `RSK-SUPA-009`; `RSK-SUPA-012`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0024` — Registrar ingreso, ubicación y reubicación mediante movimientos correlacionados                                    | `nexo`      | `IMPLEMENTED_CURRENT`      | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT                                                                                         | `RSK-SUPA-009`; `RSK-SUPA-012`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0025` — Retirar, consumir o trasladar existencias conservando unidad, conversión, origen y destino                         | `nexo`      | `IMPLEMENTED_CURRENT`      | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT                                                                                         | `RSK-SUPA-009`; `RSK-SUPA-012`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0026` — Contar como observación, investigar diferencias y ajustar mediante decisión separada                               | `nexo`      | `IMPLEMENTED_CURRENT`      | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT                                                                                         | `RSK-SUPA-009`; `RSK-SUPA-012`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0027` — Gestionar condición, vencimiento, cuarentena, merma, pérdida, frío y disposición                                   | `nexo`      | `PARTIAL_CURRENT`          | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC                                               | `RSK-SUPA-009`; `RSK-SUPA-014`; `RSK-SUPA-012`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0028` — Ejecutar abastecimiento interno de solicitud a recepción con cantidades conciliables por etapa                     | `nexo`      | `PARTIAL_CURRENT`          | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, CROSS_DOMAIN_BOUNDARY                                                  | `RSK-SUPA-009`; `RSK-SUPA-016`; `RSK-SUPA-012`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0029` — Gestionar identidad, ubicación, custodia, préstamo y transferencia de activos                                      | `nexo`      | `PARTIAL_CURRENT`          | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC                                               | `RSK-SUPA-009`; `RSK-SUPA-014`; `RSK-SUPA-012`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0030` — Gestionar mantenimiento, reparación, garantía, repuesto y disposición de activos                                   | `nexo`      | `PARTIAL_CURRENT`          | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC                                               | `RSK-SUPA-009`; `RSK-SUPA-014`; `RSK-SUPA-012`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0031` — Gestionar disponibilidad de vehículos, combustible, kilometraje e incidencias                                      | `nexo`      | `NO_CANONICAL_SOURCE`      | `MEDIO`      | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC                                                    | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-014`; `RSK-SUPA-012` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0032` — Controlar entrega, tenencia, retorno, pérdida y completitud de reutilizables y contenedores                        | `nexo`      | `PARTIAL_CURRENT`          | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC                                               | `RSK-SUPA-009`; `RSK-SUPA-014`; `RSK-SUPA-012`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0033` — Planear producción desde demanda, inventario, capacidad, prioridad y fecha requerida                               | `fogo`      | `PARTIAL_CURRENT`          | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC                                               | `RSK-SUPA-009`; `RSK-SUPA-014`; `RSK-SUPA-016`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0034` — Preparar materiales y ejecutar producción contra una versión aprobada                                              | `fogo`      | `IMPLEMENTED_CURRENT`      | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT                                                                                         | `RSK-SUPA-009`; `RSK-SUPA-016`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0035` — Inspeccionar y decidir liberación, retención, rechazo o corrección de producto                                     | `fogo`      | `NO_CANONICAL_SOURCE`      | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC                                                    | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-014`; `RSK-SUPA-016` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0036` — Empacar, etiquetar y almacenar producto terminado con trazabilidad preservada                                      | `fogo`      | `PARTIAL_CURRENT`          | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE                                                                         | `RSK-SUPA-009`; `RSK-SUPA-016`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0037` — Gestionar reproceso, aprovechamiento, rendimiento, merma y cierre productivo                                       | `fogo`      | `PARTIAL_CURRENT`          | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC                                               | `RSK-SUPA-009`; `RSK-SUPA-014`; `RSK-SUPA-016`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0038` — Gestionar servicio en mesa de apertura a cierre con pedido, preparación, entrega, pago y conciliación              | `pulso`     | `FRAGMENTED_CURRENT`       | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, CROSS_DOMAIN_BOUNDARY                                               | `RSK-SUPA-009`; `RSK-SUPA-016`; `RSK-SUPA-006`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-017                                |
| `VPROC-0039` — Gestionar venta de mostrador o para llevar con entrega y cobro correlacionados                                     | `pulso`     | `FRAGMENTED_CURRENT`       | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, CROSS_DOMAIN_BOUNDARY                                               | `RSK-SUPA-009`; `RSK-SUPA-016`; `RSK-SUPA-006`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-017                                |
| `VPROC-0040` — Normalizar pedidos de canales externos y transferirlos al proceso interno con reconciliación                       | `pulso`     | `EXTERNAL_FRAGMENTED`      | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, EXTERNAL_RECONCILIATION, CROSS_DOMAIN_BOUNDARY                      | `RSK-SUPA-009`; `RSK-SUPA-016`; `RSK-SUPA-006`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-017                                |
| `VPROC-0041` — Gestionar cotización, aprobación, capacidad, producción, facturación y entrega de catering o venta B2B             | `pulso`     | `NO_CANONICAL_SOURCE`      | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC                                                    | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-014`; `RSK-SUPA-006` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013; SUPA-ARC-017                  |
| `VPROC-0042` — Gestionar modificación, sustitución, cancelación, anulación y devolución sin confundir sus efectos                 | `pulso`     | `FRAGMENTED_CURRENT`       | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, CROSS_DOMAIN_BOUNDARY                                               | `RSK-SUPA-009`; `RSK-SUPA-016`; `RSK-SUPA-006`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-017                                |
| `VPROC-0043` — Cobrar, confirmar pago y emitir soporte fiscal mediante contrato conciliable                                       | `pulso`     | `FRAGMENTED_CURRENT`       | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, CROSS_DOMAIN_BOUNDARY                                               | `RSK-SUPA-009`; `RSK-SUPA-016`; `RSK-SUPA-006`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-017                                |
| `VPROC-0044` — Cerrar caja y conciliar ventas, pagos, efectivo, diferencias y responsables                                        | `pulso`     | `PARTIAL_CURRENT`          | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE                                                                         | `RSK-SUPA-009`; `RSK-SUPA-006`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-017                                |
| `VPROC-0045` — Identificar cliente y administrar fidelización mediante ledgers y consentimientos separados                        | `pass`      | `FRAGMENTED_CURRENT`       | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, CROSS_DOMAIN_BOUNDARY                                               | `RSK-SUPA-009`; `RSK-SUPA-016`; `RSK-SUPA-006`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-017                                |
| `VPROC-0046` — Gestionar reclamo, devolución, compensación y aprendizaje de causa                                                 | `pulso`     | `FRAGMENTED_CURRENT`       | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, NO_CANONICAL_COMMAND_RPC, CROSS_DOMAIN_BOUNDARY                     | `RSK-SUPA-009`; `RSK-SUPA-014`; `RSK-SUPA-016`; `RSK-SUPA-006` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013; SUPA-ARC-017                  |
| `VPROC-0047` — Gestionar reservas, eventos y comunicaciones al cliente con capacidad y consentimiento                             | `pulso`     | `NO_CANONICAL_SOURCE`      | `MEDIO`      | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE                                                                              | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-006`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-017                                |
| `VPROC-0048` — Planear ruta, vehículo, carga, secuencia y restricciones antes del despacho                                        | `nexo`      | `PARTIAL_CURRENT`          | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC                                               | `RSK-SUPA-009`; `RSK-SUPA-014`; `RSK-SUPA-012`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0049` — Ejecutar ruta y confirmar entrega, rechazo, novedad o retorno con prueba suficiente                                | `nexo`      | `PARTIAL_CURRENT`          | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE                                                                         | `RSK-SUPA-009`; `RSK-SUPA-012`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0050` — Integrar entrega de tercero con seguimiento, prueba y conciliación interna                                         | `pulso`     | `EXTERNAL_FRAGMENTED`      | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, EXTERNAL_RECONCILIATION, CROSS_DOMAIN_BOUNDARY                      | `RSK-SUPA-009`; `RSK-SUPA-016`; `RSK-SUPA-006`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-017                                |
| `VPROC-0051` — Registrar hechos económicos desde eventos operativos y soportes correlacionados                                    | `numera`    | `PARTIAL_CURRENT`          | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, CROSS_DOMAIN_BOUNDARY                                                  | `RSK-SUPA-009`; `RSK-SUPA-016`; `RSK-SUPA-006`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-017                                |
| `VPROC-0052` — Gestionar obligación, aprobación y pago a proveedor con conciliación bancaria                                      | `numera`    | `NO_CANONICAL_SOURCE`      | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC                                                    | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-014`; `RSK-SUPA-006` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013; SUPA-ARC-017                  |
| `VPROC-0053` — Gestionar cartera, cobro, recaudo, aplicación y diferencia                                                         | `numera`    | `NO_CANONICAL_SOURCE`      | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC                                                    | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-014`; `RSK-SUPA-006` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013; SUPA-ARC-017                  |
| `VPROC-0054` — Gestionar costos, distribución, presupuesto, cierre y rentabilidad con reglas versionadas                          | `numera`    | `PARTIAL_CURRENT`          | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE                                                                         | `RSK-SUPA-009`; `RSK-SUPA-006`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-017                                |
| `VPROC-0055` — Gestionar limpieza, inspección, mantenimiento, plagas, servicios y cierre de novedades de instalaciones            | `nexo`      | `NO_CANONICAL_SOURCE`      | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC                                                    | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-014`; `RSK-SUPA-012` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0056` — Gestionar contenido y promociones desde solicitud y aprobación hasta publicación y retiro                          | `aura`      | `FUTURE_OR_PARTIAL`        | `MEDIO`      | NO_CANONICAL_EVENT_TRANSPORT, DEFERRED_OR_FUTURE, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC, OWNER_IMPLEMENTATION_GAP | `RSK-SUPA-009`; `RSK-SUPA-014`; `RSK-SUPA-039`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013; paquetes E5 de la propietaria |
| `VPROC-0057` — Convertir consultas y oportunidades de canales digitales en casos comerciales trazables                            | `aura`      | `FUTURE_OR_PARTIAL`        | `MEDIO`      | NO_CANONICAL_EVENT_TRANSPORT, DEFERRED_OR_FUTURE, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC, OWNER_IMPLEMENTATION_GAP | `RSK-SUPA-009`; `RSK-SUPA-014`; `RSK-SUPA-039`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013; paquetes E5 de la propietaria |
| `VPROC-0058` — Gestionar solicitudes e incidentes tecnológicos con diagnóstico, prioridad, resolución y conocimiento              | `viso`      | `PARTIAL_CURRENT`          | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE                                                                         | `RSK-SUPA-009`; `RSK-SUPA-011`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0059` — Gestionar el ciclo de acceso tecnológico desde solicitud hasta revocación y verificación                           | `viso`      | `PARTIAL_CURRENT`          | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, CROSS_DOMAIN_BOUNDARY                                                  | `RSK-SUPA-009`; `RSK-SUPA-016`; `RSK-SUPA-011`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0060` — Gestionar documentos y evidencia desde creación hasta disposición con metadatos y custodia                         | `viso`      | `FRAGMENTED_CURRENT`       | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, FRAGMENTED_SOURCE, NO_CANONICAL_COMMAND_RPC, CROSS_DOMAIN_BOUNDARY                     | `RSK-SUPA-009`; `RSK-SUPA-014`; `RSK-SUPA-016`; `RSK-SUPA-011` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0061` — Gestionar medición, análisis, decisión de mejora y verificación de resultado                                       | `numera`    | `PARTIAL_CURRENT`          | `MEDIO`      | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE, NO_CANONICAL_COMMAND_RPC                                               | `RSK-SUPA-009`; `RSK-SUPA-014`; `RSK-SUPA-006`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013; SUPA-ARC-017                  |
| `VPROC-0062` — Gestionar continuidad desde detección hasta operación mínima, recuperación, reconciliación y aprendizaje           | `viso`      | `NO_CANONICAL_SOURCE`      | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE                                                                              | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-011`                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016                                              |
| `VPROC-0063` — Gestionar riesgos empresariales como registro versionado de riesgo, tratamiento y seguimiento                      | `viso`      | `NO_CANONICAL_SOURCE`      | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC                                                    | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-014`; `RSK-SUPA-011` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0064` — Gestionar relaciones con asesores y autoridades conservando obligaciones, vencimientos y evidencia interna         | `viso`      | `NO_CANONICAL_SOURCE`      | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC                                                    | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-014`; `RSK-SUPA-011` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0065` — Acompañar desempeño y desarrollo con objetivos, retroalimentación, privacidad y decisiones separadas               | `viso`      | `NO_CANONICAL_SOURCE`      | `MEDIO`      | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC                                                    | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-014`; `RSK-SUPA-011` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0066` — Entregar y controlar elementos de protección desde requisito hasta devolución y evidencia                          | `viso`      | `NO_CANONICAL_SOURCE`      | `CRÍTICO`    | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC                                                    | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-014`; `RSK-SUPA-011` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0067` — Gestionar kits y conjuntos sin confundir kit, activo, LPN o contenedor                                             | `nexo`      | `NO_CANONICAL_SOURCE`      | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC                                                    | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-014`; `RSK-SUPA-012` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013                                |
| `VPROC-0068` — Medir satisfacción separando medición, incentivo, reclamo y compensación                                           | `pulso`     | `NO_CANONICAL_SOURCE`      | `MEDIO`      | NO_CANONICAL_EVENT_TRANSPORT, NO_SOURCE, NO_CANONICAL_COMMAND_RPC                                                    | `RSK-SUPA-009`; `RSK-SUPA-013`; `RSK-SUPA-014`; `RSK-SUPA-006` | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-013; SUPA-ARC-017                  |
| `VPROC-0069` — Gestionar presupuestos con versión, supuestos, aprobación, vigencia, consumo, proyección y desviación              | `numera`    | `PARTIAL_CURRENT`          | `ALTO`       | NO_CANONICAL_EVENT_TRANSPORT, PARTIAL_SOURCE                                                                         | `RSK-SUPA-009`; `RSK-SUPA-006`                                 | SUPA-ARC-002; SUPA-ARC-003; SUPA-ARC-016; SUPA-ARC-017                                |

Resumen de exposición por proceso:

| Clase máxima    |                                                                                      Procesos |
| --------------- | --------------------------------------------------------------------------------------------: |
| `CRÍTICO`       |                                                                                        **35** |
| `ALTO`          |                                                                                        **26** |
| `MEDIO`         |                                                                                         **8** |
| `DEUDA_TÉCNICA` | **0** — la deuda se registra como riesgo transversal y no como única exposición de un proceso |
| **Total**       |                                                                                        **69** |

#### 9. Gates de salida de la auditoría

| Gate                                      | Riesgos cubiertos                                                                   | Condición mínima                                                                     | Tareas propietarias                                                                              |
| ----------------------------------------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------ |
| `GATE-RISK-A — Contención crítica`        | privilegios, secretos, identidad, privacidad, pagos, acceso y automatismos críticos | contención demostrada, pruebas negativas, monitoreo y prohibición de expansión       | `SUPA-ARC-006`; `SUPA-ARC-008` a `SUPA-ARC-010`; `SUPA-ARC-014` a `SUPA-ARC-016`; `SUPA-ARC-020` |
| `GATE-RISK-B — Arquitectura objetivo`     | dominios, propietarios, public, vistas, RPC, eventos, Storage, Realtime y entornos  | decisión aprobada por objeto y proceso sin fuentes competidoras                      | `SUPA-ARC-001` a `SUPA-ARC-024`                                                                  |
| `GATE-RISK-C — Preparación de transición` | drift, legacy, calidad, tipos, rendimiento, retención y consumidores                | mapa de dependencias, backfill, compatibilidad, pruebas, rollback y ambiente         | `SUPA-TRANS-001` a `SUPA-TRANS-014`                                                              |
| `GATE-RISK-D — Ejecución controlada`      | cambios físicos y retiro                                                            | roadmap ejecutable, CI bloqueante, evidencia pre y post, reconciliación y aprobación | `SUPA-TRANS-015`; paquetes E5; BLOQUE R                                                          |

#### 10. Prioridad de tratamiento

##### 10.1. Contención antes de arquitectura

`RSK-SUPA-001` a `RSK-SUPA-005` requieren que cualquier superficie relacionada permanezca sin expansión hasta que una tarea posterior defina y pruebe la contención. Esta clasificación no ejecuta rotaciones, revocaciones, cambios de políticas ni despliegues.

##### 10.2. Decisiones fundacionales de arquitectura

`RSK-SUPA-004`, `006`, `011`, `012`, `016`, `017`, `018`, `021`, `033` y `036` deberán informar desde `SUPA-ARC-001` la separación administrado-empresarial, los dominios, propietarios, superficies, contratos y tipos.

##### 10.3. Transición y retiro

`RSK-SUPA-007`, `008`, `022`, `023`, `024`, `025`, `026`, `035`, `041`, `042`, `043`, `046`, `053` y `054` no autorizan cambios inmediatos; deberán resolverse mediante dependencias, compatibilidad, backfill, prueba, rollback y retiro progresivo.

##### 10.4. Readiness operativo

`RSK-SUPA-009`, `010`, `015`, `019`, `020`, `027`, `028`, `029`, `030`, `031`, `032`, `034`, `037`, `038`, `040`, `044`, `045`, `048`, `049`, `050`, `051`, `052`, `055` y `056` deberán producir gates verificables de integración, seguridad, capacidad, observabilidad y gobierno antes de producción.

#### 11. Tratamiento de aceptación y riesgo residual

Una tarea posterior solo podrá aceptar temporalmente un riesgo cuando registre:

- autoridad competente;
- justificación empresarial concreta;
- alcance y ambientes;
- controles compensatorios probados;
- monitoreo y umbral de alerta;
- fecha de expiración;
- tarea y gate de resolución;
- evidencia de revisión periódica.

La aceptación no cambia la identidad del riesgo ni elimina sus TREQ. Al vencer, el riesgo vuelve a bloquear la superficie si no existe renovación aprobada con evidencia nueva.

#### 12. Decisiones que esta tarea no autoriza

- rotar, revocar, revelar o mover secretos;
- modificar schemas, tablas, columnas, constraints, índices, funciones, triggers o políticas;
- cambiar Data API, RLS, grants, Auth, Storage, Realtime, Edge Functions o cron;
- crear outbox, colas, workers, vistas, RPC o tipos;
- ejecutar backfills, merges, anonimización, borrado o retiro legacy;
- aceptar, cerrar o degradar riesgos;
- iniciar implementación física o desplegar paquetes.

#### 13. Hallazgos de cierre de la auditoría

1. La principal exposición sistémica no es el volumen de objetos sino la combinación de autoridad incompleta, superficies privilegiadas, drift y contratos interaplicación no materializados.
2. La existencia de 379 relaciones y 347 funciones no equivale a cobertura de los 69 procesos: 20 carecen de fuente canónica y 24 de comando canónico.
3. Los 395 eventos funcionales aprobados no tienen transporte físico VPROC, por lo que la coordinación actual depende de mecanismos heterogéneos.
4. Los riesgos críticos de secretos, privilegios, identidad, pagos, drift, recuperación, eventos, automatización, inventario y fuentes de verdad deberán condicionar la arquitectura y la transición.
5. La deuda técnica queda separada de los riesgos materiales para evitar que mantenibilidad o documentación reduzcan artificialmente exposiciones activas.
6. Todos los riesgos tienen tarea y gate; no quedan pendientes narrativos sin dueño documental.

#### 14. Requisitos de prueba derivados

Se incorporan **40** requisitos al registro canónico completo:

```text
TREQ-SUPABASE-491 a TREQ-SUPABASE-530
```

El detalle normativo reside únicamente en `04A_REGISTRO_CANONICO_DE_REQUISITOS_DE_PRUEBA.md`.

#### 15. Manifiesto de integridad

| Elemento                     | Valor                                                                                           |
| ---------------------------- | ----------------------------------------------------------------------------------------------- |
| base `04A` de `SUPA-AUD-023` | SHA-256 `3e3209f610729c2ef91606c9b090df1020dfe8815ecbb7e0c3efdbcfc6cf6ce4`; **4785** requisitos |
| tarea `SUPA-AUD-023`         | SHA-256 `1ad664acd733e4b06ff65f7fcc4b940e6302533197f7652178864c64b61c7cad`                      |
| bloque propietario remoto    | blob `60b442fdf3c50bb4bf932de1646d101c6c05f2cf`                                                 |
| commit remoto observado      | `939071fc7d4c173e01f15cf10312af0740afe447`                                                      |
| riesgos consolidados         | **56**                                                                                          |
| distribución                 | **16 CRÍTICOS; 20 ALTOS; 10 MEDIOS; 10 DEUDA_TÉCNICA**                                          |
| procesos clasificados        | **69**                                                                                          |
| nuevos TREQ                  | `TREQ-SUPABASE-491` a `TREQ-SUPABASE-530`                                                       |

#### 16. Criterios de aceptación

- [ ] Existe un único registro `SUPABASE-CONSOLIDATED-RISK-REGISTER-001@1.0.0`.
- [ ] Hay exactamente 56 riesgos con identidad única y una clase primaria.
- [ ] La distribución es 16 críticos, 20 altos, 10 medios y 10 de deuda técnica.
- [ ] Cada riesgo conserva evidencia, superficie, impacto, probabilidad, detectabilidad, controles, resolución, gate y TREQ.
- [ ] Los 69 procesos aparecen una vez y conservan propietaria y cobertura de `SUPA-AUD-023`.
- [ ] Ningún proceso queda sin clase máxima ni riesgos primarios.
- [ ] No existe riesgo sin tarea exacta de resolución.
- [ ] No se confunde prioridad TREQ con severidad del riesgo.
- [ ] Ningún control parcial se presenta como cierre o aceptación.
- [ ] El `04A` conserva 4785 filas históricas y añade 40 filas consecutivas.
- [ ] No se autoriza ninguna mutación física ni aceptación de riesgo.
- [ ] La continuidad pasa exclusivamente a `SUPA-ARC-001`.
...
`SUPA-AUD-024` cierra la secuencia de auditoría `SUPA-AUD-001` a `SUPA-AUD-024`. No inicia arquitectura, transición ni implementación física.
