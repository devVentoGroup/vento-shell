...
select is(
  (select count(*) from auth_db_018_rpc_contracts),
  100::bigint,
  'AUTH-DB-018 reuses exactly 100 canonical RPC contracts from AUTH-SRV-003'
);
...
select is(
  (
    ...
    where n.nspname in ('app_private','club','pass','payments','pos','public','talento','viso')
      and c.relkind = 'v'
  ),
  62::bigint,
  'current Vento view inventory remains 62'
);
...
select is(
  ...
  58::bigint,
  '58 source views use security_invoker'
);
...
select is(
  ...
  4::bigint,
  'four privileged source views remain blocked as-is'
);
...
select is(
  ...
  224::bigint,
  'current direct Vento routine inventory remains 224'
);
...
select is(
  ...
  166::bigint,
  'predecessor-adjusted direct SECURITY DEFINER inventory is 166'
);
...
select is(
  ...
  73::bigint,
  'current trigger function inventory remains 73'
);
...
select is(
  ...
  4::bigint,
  'four extension routines are explicitly platform-managed'
);

select is(
  62::bigint + 224::bigint,
  286::bigint,
  'current primary classification universe is exactly 286 identities'
);

select is(
  (
    select count(distinct r.rpc_schema || '.' || r.rpc_name)
  ...
  93::bigint,
  '93 canonical RPC contracts still exist in the current catalog'
);
...
select is(
  ...
  7::bigint,
  'seven historical consumed RPC contracts are absent and are not recreated by inference'
);
...
select is(
  ...
  83::bigint,
  '83 current canonical RPC contracts remain client-executable candidates'
);
...
select is(
  ...
  10::bigint,
  '10 current canonical RPC contracts remain server-only and are not copied to api'
);

select ok(
  to_regnamespace('api') is not null,
  'api schema exists from AUTH-DB-016 foundation'
);
...
select is(
  ...
  57::bigint,
  'api contains exactly 57 canonical security-invoker read views after preserving the public compatibility duplicate'
);
...
select is(
  ...
  (
    ...
    from pg_catalog.pg_proc p
    join pg_catalog.pg_namespace n on n.oid = p.pronamespace
    join auth_db_018_rpc_contracts r
  ...
  'api contains one wrapper for every current client-executable canonical RPC signature'
);
...
select is(
  ...
  0::bigint,
  'api contains zero SECURITY DEFINER functions'
);
...
select is(
  ...
  0::bigint,
  'api contains zero trigger functions'
);
...
select is(
  ...
  0::bigint,
  'api contains zero authoritative tables partitions or sequences'
);

select ok(
  to_regclass('api.permission_catalog_human_v1') is null,
  'privileged permission_catalog_human_v1 is not published as-is'
);

select ok(
  to_regclass('api.shared_operational_device_actor_policies_admin_v1') is null,
  'privileged shared_operational_device_actor_policies_admin_v1 is not published as-is'
);

select ok(
  to_regclass('api.shared_operational_device_templates_admin_v1') is null,
  'privileged shared_operational_device_templates_admin_v1 is not published as-is'
);

select ok(
  to_regclass('api.shared_operational_devices_admin_v1') is null,
  'privileged shared_operational_devices_admin_v1 is not published as-is'
);

select ok(
  not has_schema_privilege('anon', 'api', 'USAGE'),
  'AUTH-DB-018 does not advance anon api USAGE from AUTH-DB-017'
);

select ok(
  not has_schema_privilege('authenticated', 'api', 'USAGE'),
  'AUTH-DB-018 does not advance authenticated api USAGE from AUTH-DB-017'
);
