-- FOGO 1.1
-- Empaques reales de producción como stock físico de lote.
--
-- Decisión:
-- - Los insumos comprados usan product_uom_profiles manuales.
-- - Las preparaciones/productos producidos NO dependen de presentaciones manuales.
-- - Cada fila de production_batch_packages representa un empaque físico real producido.
-- - Remisiones/conteos deben consumir estos empaques completos o fraccionarlos dejando remanente.
...
ALTER TABLE public.production_batch_packages
  ...
  ADD COLUMN IF NOT EXISTS reserved_qty numeric DEFAULT 0 NOT NULL,
...
UPDATE public.production_batch_packages package
SET
  site_id = coalesce(package.site_id, batch.site_id),
  location_id = coalesce(package.location_id, batch.destination_location_id),
  original_qty = coalesce(package.original_qty, package.actual_qty),
  remaining_qty = coalesce(
    package.remaining_qty,
    case
      when package.status in ('available', 'reserved') then package.actual_qty
      else 0
    end
  ),
  reserved_qty = coalesce(package.reserved_qty, 0)
FROM public.production_batches batch
WHERE batch.id = package.batch_id;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'production_batch_packages_remaining_qty_chk'
      AND conrelid = 'public.production_batch_packages'::regclass
  ) THEN
    ALTER TABLE public.production_batch_packages
      ADD CONSTRAINT production_batch_packages_remaining_qty_chk
      CHECK (remaining_qty IS NULL OR remaining_qty >= 0) NOT VALID;
  END IF;

  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'production_batch_packages_reserved_qty_chk'
      AND conrelid = 'public.production_batch_packages'::regclass
  ) THEN
    ALTER TABLE public.production_batch_packages
      ADD CONSTRAINT production_batch_packages_reserved_qty_chk
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'production_batch_packages_original_qty_chk'
      AND conrelid = 'public.production_batch_packages'::regclass
  ) THEN
    ALTER TABLE public.production_batch_packages
      ADD CONSTRAINT production_batch_packages_original_qty_chk
      CHECK (original_qty IS NULL OR original_qty > 0) NOT VALID;
  END IF;
END $$;
