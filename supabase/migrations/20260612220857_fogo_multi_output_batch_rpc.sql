...
begin
  ...
  v_primary_product_id := v_recipe.product_id;
  ...
  loop
    ...
    if v_output.product_id = v_recipe.product_id then
      ...
      v_primary_pct := coalesce(v_output.cost_allocation_pct, 0);
    ...
    if v_batch.output_mode in ('inventory_stock', 'sellable_stock') then
      ...
      if v_output.product_id <> v_recipe.product_id then
        ...
        values (v_output.destination_location_id, v_output.product_id, v_output.produced_qty, now())
        ...
        set avg_unit_cost = coalesce(v_unit_cost, avg_unit_cost),
            updated_at = now()
    ...
    insert into public.production_batch_outputs (
      ...
      v_unit_cost,
      v_movement_id,
      jsonb_build_object('source', 'fogo_create_real_production_batch')
    );
  end loop;
