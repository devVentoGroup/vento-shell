...
begin
  ...
  if coalesce(p_invoice_requested, false) then
    ...
    if v_legal_name is null then
      raise exception 'billing_legal_name_required';
    end if;

    if v_document_type is null then
      raise exception 'billing_document_type_required';
    end if;

    if v_document_number is null then
      raise exception 'billing_document_number_required';
    end if;
  ...
  if v_order_site_id is null then
