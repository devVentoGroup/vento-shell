...
before insert or update of catalog_item_id, commercial_collection_id
...
comment on table pass.catalog_item_collections is
  'Relacion muchos-a-muchos entre productos comerciales y colecciones. commercial_collection_id en catalog_items se conserva temporalmente como coleccion principal compatible.';

comment on column pass.catalog_item_collections.is_primary is
  'Coleccion principal usada por interfaces antiguas mientras se completa la migracion.';

grant select, insert, update, delete on pass.catalog_item_collections to authenticated, service_role;
